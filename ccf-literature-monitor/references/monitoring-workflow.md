...
- If the user wants novelty check, read the current idea from `ccfa.yaml` when available. Err on the side of inclusion for potential overlap: if a new paper has a similar problem, insight, mechanism, dataset, or paper conclusion, report it with uncertainty rather than hiding it.
...
- For each high-relevance candidate, read at least the title and abstract. Do not report high overlap without checking the abstract and, when accessible, the introduction or method summary.
...
- Compare each new paper's research question and main conclusions with the user's idea.
- Check six axes:
  - problem/task,
  - stated gap,
  - core mechanism,
...
- Classify each overlap:
  ...
  - **Low (RESEARCH):** shared motivation or setting, but different method or conclusion; retrieve if relevant.
  - **Medium (RESEARCH):** overlapping mechanism, benchmark, or conclusion; deep search and differentiation are needed.
...
4 = same problem plus similar mechanism or conclusion
