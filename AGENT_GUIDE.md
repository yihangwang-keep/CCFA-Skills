...
16. Route a complete initial scientific-problem document to `ccf-mes-validation` for a minimal-but-complete MES, environment audit, initial algorithm, and repair loop. Route a post-anchor upgrade request to `ccf-complexity-upgrade`; it reads current code/results, writes the upgrade scenario document, modifies the existing environment, audits it, and then modifies/repairs the algorithm. Environment and algorithm evidence goes to the two independent code auditors.
