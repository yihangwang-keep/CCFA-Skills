...
16. Route communication/networking objective design, constraint modeling, scenario abstraction, task-communication formulation, joint-optimization necessity checks, explainability checks, and "is this a toy problem?" environment audits to `ccf-experiment-env-design`; route full baselines, metrics, ablations, and result-table plans to `ccf-experiment-designer` after the formulation is credible.
