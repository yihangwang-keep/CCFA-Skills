...
16. Route ordered communication scenario and MVP design to `ccf-env-design`, environment implementation evidence to `ccf-env-code-auditor`, formal algorithm and MVP design to `ccf-algorithm-designer`, and algorithm implementation evidence to `ccf-algorithm-code-auditor`. Route failed MVP runs to `ccf-experiment-debugger`, which refreshes auditor evidence, isolates the owner, coordinates one minimal change, and closes affected reruns. Keep baselines, metrics, ablations, and paper-scope result plans in `ccf-experiment-designer`.
