...
If the candidate MES cannot execute the formal problem or the formal problem is independently shown infeasible/ill-posed before acceptance, revise the problem and candidate MES at the environment-design layer and rerun the design gates. Do not hand an unresolved candidate to algorithm design. Once accepted, freeze the anchor and use the complexity ladder below.

Once L1/L2 acceptance closes, mark this MES as the `anchor` and freeze its version. The anchor is the paper's minimum scale but already a complete instance of the scientific problem. Subsequent increases in scale, load, topology, uncertainty, coupling, or boundary stress are recorded as method-independent complexity stages against the same anchor and interface. Every stage keeps anchor cases as regressions; a failed stage routes to algorithm code/design repair and does not authorize a new MES.
...
**L2 probe contract (anchor-only):** before comparative outcomes for the initial anchor algorithm, predeclare a joint-tradeoff or optimization target, its domain/formal basis, and independent evidence that the target region is feasible and attainable under the MES contract. Define representative fixed/static, domain, greedy/myopic, decoupled, low-dimensional tuned, and random-feasible `environment_probe` policies as applicable; disclose tuning ranges and search budgets; and match decision-time information, feasibility enforcement, stopping conditions, cases, and resource accounting. An impossible or unsupported target cannot establish algorithmic need. This contract is not rerun for later complexity stages.

`ccf-env-code-auditor` freezes and executes this contract once for the anchor. It sets `algorithmic_need: demonstrated` only when L1 passes and adequately tuned representative probes still cannot reach the independently justified attainable target. If a probe reaches it, the environment may remain `environment-valid`, but the auditor sets `not_demonstrated`. Missing target reachability, tuning, coverage, or fairness evidence is `insufficient_evidence`. For a later complexity stage, carry the anchor result forward unchanged and run only the stage L1 consistency, anchor regression, and current algorithm checks. Never change the MES, add unsupported difficulty, or select favorable cases merely to make a probe fail.
...
## 10. Complexity Ladder, Evolution, And Invalidation

Bind the paper-scenario, formal-problem, parameter-range, frozen anchor MES, complexity-stage, implementation, validation-contract, and evidence-epoch versions. Accepted authority artifacts are immutable. When code, audit, or experiment evidence motivates a change, classify it before editing as `implementation_repair`, `evidence_expansion`, `complexity_expansion`, legacy/exception `scenario_extension`, `formal_amendment`, or `research_reframe` using `scenario-evolution-contract.md`.

`complexity_expansion` keeps the formal problem, parameter-range semantics, anchor MES, and decision interface unchanged. It adds one user-requested, method-independent complexity dimension or operating stage, records its parent stage and delta, and runs the stage implementation plus anchor regressions. It is evidence expansion, not MES redesign, and it does not reopen L2 or change `algorithmic_need`. If the added setting changes objective semantics, material constraints, information timing, feasibility meaning, or formal construction semantics, reclassify it as a formal amendment before running comparative evidence.
...
When environment L1 passes and algorithm code matches its specification, preserve a repair-exhaustion record covering attempted mechanism or family revisions at the failing complexity stage, reference/bound evidence, unchanged failures, and why another algorithm-only delta is no longer credible. This record triggers environment and mathematical-model review; it does not automatically authorize a change or permit changing the anchor MES.
...
- **formal-model defect:** the authoritative problem is infeasible or ill-posed, has inconsistent information timing, omits a necessary causal factor, or otherwise fails to represent the paper scenario; stop the complexity ladder, create the smallest justified formal correction and a new candidate MES/evidence epoch, and preserve the old anchor as historical evidence;
