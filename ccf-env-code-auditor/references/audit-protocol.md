...
current paper-scenario, formal-problem, parameter-range, and minimum executable
scenario (MES) versions. Load
...
versions, MES version, environment implementation digests, configuration,
...
range, MES, equation, environment-code, entry-point, configuration, seed,
...
Freeze the MES version, evaluation cases, decision-time information,
feasibility rules, seeds, resource accounting, tuning budget, and target before
running the sweep. Record the target's units, domain or formal basis, and
...
Record Layer 2 in the internal ledger:
...
`evidence_status: complete` means target attainability is supported and the
predeclared sweep finished with matched information, feasibility, cases, and
budget; it does not mean that every probe failed. A successful probe yields `complete` with
`algorithmic_need: not_demonstrated`. Use `incomplete` or `not_run` with
`insufficient_evidence` when tuning or fairness evidence is missing, `invalid`
for an invalid sweep, and `stale` after an authority or artifact change. Never
mark the procedure invalid merely because a heuristic performed well.
...
preserve attempted repairs and references, and forward a model-review request
...
meaning, or applicability, create a new environment version and invalidate all
dependent MES, algorithm, baseline, and experimental evidence.

## 8. Verdict And Readiness

- pass: Layer 1 authority/design/code contracts and applicable native review axes are current, and Layer 2 has `evidence_status: complete` under the predeclared target and budget; `algorithmic_need` remains a separate scientific outcome;
- conditional: no contradiction is found, but a decisive Layer 1 trace, configuration, native review, or Layer 2 target-basis, attainability, tuning, coverage, or sweep item is incomplete;
...
- algorithmic-need: `demonstrated` only when Layer 2's properly tuned, predeclared probe sweep misses its target; `not_demonstrated` when a representative heuristic reaches the target; `contradicted`, `insufficient_evidence`, or `stale` when supported by the current record;
