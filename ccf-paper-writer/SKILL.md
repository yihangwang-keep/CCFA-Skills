...
description: "Plan, draft, revise, polish, and reviewer-proof CCF research paper text while preserving user-supplied idea scope and evidence. Use for paper writing, abstract/introduction/related work/method/experiment writing, ????, story line, claim-evidence alignment. Do not perform full scientific review or rebuttal as the main task."
metadata:
  ccf_skill_controls:
...
If the task is primarily literature search, experiment design, or length compression, route to `ccf-literature-searcher`, `ccf-experiment-designer`, or `ccf-paper-compressor` by handoff mode instead of absorbing those tasks into writing. Rebuttal is not part of the writing loop; use `ccf-rebuttal-writer` only when the user explicitly asks for rebuttal, author response, response letter, resubmission response, or 审稿意见回复.
...
5. If Related Work, Introduction, novelty, datasets, or baselines require current sources, follow CCFA handoff mode before using `ccf-literature-searcher`. If denied, mark citation and novelty risks rather than inventing sources.
...
9. Run the score-risk loop before finalizing or whenever the user reports weak scores. Load `references/score-lifting-loop.md`, identify likely reviewer deductions, classify fixability, revise, and re-score. Follow the CCFA handoff mode before using `ccf-scientific-reviewer` as an optional scientific scoring and AC/meta-review module, or `ccf-writing-reviewer` as a writing-only review module; if denied, use the local score-risk checklist only.
