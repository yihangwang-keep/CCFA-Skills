...
Let y = x + z where z ∼N(0, 2Id). The density p(y) of noisy images is then related to p(x)
through marginalization over x:
p(y) =
p(y|x) p(x) dx =
g(y −x) p(x) dx,
(1)
where g(z) is the density of z. Hence, p(y) is obtained by convolving p(x) with a Gaussian with
standard deviation . The family of densities {p(y); 0} forms a scale-space representation of
p(x), analogous to the temporal evolution of a diffusion process.
Diffusion models learn an approximation sθ(y) (dropping the dependence for simplicity) of the
scores ∇log p(y) of the blurred densities p(y) at all noise levels . The collection of these score
...
using eq. (3): sθ(y) = (fθ(y) −y)/2. As we show in Appendix D.2, the error in estimating the
...
can then diagonalize it to obtain eigenvalues (k(y))1kd and eigenvectors (ek(y))1kd.
...
k(y) ⟨y, ek(y)⟩ek(y).
(6)
The denoiser can thus be interpreted as performing shrinkage with factors k(y) along axes of a basis
...
Even if the denoiser is not bias-free, small eigenvalues k(y) reveal local invariances of the denoising
...
they are naturally enforced by minimizing mean squared error (MSE) as expressed with Steins
unbiased risk estimate (SURE, proved in Appendix D.3 for completeness):
MSE(f, 2) = E
22 tr ∇f(y) + ∥y −f(y)∥2 −2d
(7)
To minimize MSE, the denoiser must trade off the approximate rank of the Jacobian (the trace is the
sum of the eigenvalues) against an estimate of the denoising error: ∥y −f(y)∥2 −2d. The denoiser
...
k(y))1kd is not known. Rather than aiming for exact
optimality, classical analyses (Donoho, 1995) thus focus on the asymptotic decay of the denoising
error as the noise level 2 falls, up to multiplicative constants. This corresponds to finding a basis
(ek(y))1kd which captures the asymptotic slope of the PSNR plots in Figure 1 but not necessarily
...
basis ek but can adapt its shrinkage factors k(y). An unreachable lower-bound on the denoising
errorand thus an upper-bound on the PSNR slopeis obtained by evaluating the performance of an
oracle denoiser where the shrinkage factors k depend on the unknown clean image x rather than
the noisy observation y (Mallat, 2008). Appendix D.4 shows that the denoising error of this oracle is
(1 −k(x))2⟨x, ek⟩2 + k(x)22#
(11)
which is minimized when k(x) =
⟨x,ek⟩2
⟨x,ek⟩2+2 . The coefficient k(x) thus acts as a soft threshold:
k(x) 1 when the signal dominates the noise and k(x) 0 when the signal is weaker than the
noise. Appendix D.4 then shows that the oracle denoising error is the expected value of
k(x) =
2⟨x, ek⟩2
⟨x, ek⟩2 + 2 ∼
min(⟨x, ek⟩2, 2) = M2 + ∥x −xM∥2,
(12)
where xM = P
⟨x,ek⟩2>2⟨x, ek⟩ek is the M-term approximation of x with the M basis coefficients
...
coefficients decay as ⟨x, ek⟩2 ∼k−(+1) (up to reordering), Appendix D.4 shows that
M2 + ∥x −xM∥2 ∼2/(+1),
(13)
...
shrinkage factors k(y) by comparing ⟨y, ek⟩2 (rather than ⟨x, ek⟩2) with a threshold proportional
to 2 (Donoho & Johnstone, 1994), and achieves the decay (13) up to a logarithmic factor. The
decay (13) of the MSE with decreasing corresponds to an asymptotic slope of /( + 1) in the
PSNR curve when the input PSNR increases. Thus, a larger sparsity/regularity exponent , which
corresponds to a faster decay of the small coefficients of x in the basis (ek)1kd, leads to improved
...
left. Clean, noisy ( = 0.15) and denoised images. Bottom left. Decay of shrinkage values k(y)
...
the contours and within smooth regions of the image, whose frequency increases as k(y) decreases.
Best adaptive bases.
Adapting the basis (ek)1kd to the noisy image y allows obtaining sparser
...
decay as k−(+1), then the asymptotic PSNR slope is again /( + 1). The best oracle basis satisfies
...
curves for various regularity levels . The empirical slopes closely match the theoretical optimal
slopes (parenthesized values, dashed lines). Right. A C image ( = 4) of size 80 80 and its top
...
consider the so-called geometric C class of images (Korostelev & Tsybakov, 1993; Donoho, 1999;
Peyré & Mallat, 2008) which consist of regular contours on regular backgrounds, where the degree of
regularity is controlled by . Examples of these images are shown in Figure 4 and Appendix C.1. A
mathematical definition and an algorithm for their synthesis are presented in Appendix E.
Optimal sparse representations of C images are obtained with bandlet bases (Peyré & Mallat,
2008). Bandlets are harmonic functions oscillating at different frequencies, whose geometry is adapted
to the directional regularity of images along contours. Geometric C images can be represented
with few bandlets having low-frequency oscillations in regular regions and along contours but sharp
variations across contours. The k-th coefficient in the best bandlet basis then decays as k−(+1). It
follows that the optimal denoiser has a PSNR which asymptotically increases with a slope /( + 1)
...
spanned by deformations of x along these five dimensions. When the noise level is much smaller
...
shrinkage factors of = 0, but are otherwise unconstrained. The optimal MSE is asymptotically
...
ground/background intensity. Top center. Clean, noisy ( = 0.04), and denoised images. Bottom
center. The decay of shrinkage factors k(y) and coefficients ⟨x, ek(y)⟩indicates that the network
...
the C class of images, such geometric bases are known to be optimal, and DNN denoisers achieve
...
USENIX Security Symposium (USENIX Security 23), pp. 52535270, 2023.
...
Processing, 91:27432753, 2011.
Bradley Efron. Tweedies formula and selection bias. J American Statistical Association, 106(496):
16021614, Dec 2011.
...
probability models of images. In Intl Conf on Learning Representations (ICLR), Kigali, Rwanda,
May 2023.
...
NY, 1993.
Z Liu, P Luo, X Wang, and X Tang. Deep learning face attributes in the wild. In Proc Intl Conference
...
theoretical. IEEE signal processing magazine, 30(1):106128, 2012.
...
blind image denoising via bias-free convolutional neural networks. In Intl Conf on Learning
...
IEEE/CVF Conference on Computer Vision and Pattern Recognition, pp. 60486058, 2023.
...
score-based diffusion models. Advances in Neural Information Processing Systems, 34:14151428,
...
generalization. Advances in neural information processing systems, 33:46974708, 2020.
...
(7):31423155, 2017. doi: 10.1109/TIP.2017.2662206.
...
2015) downsampled to 8080 resolution. For experiments shown in Figures 9 and 10, we use images
...
(0, ), this method uses two hyperparameters h ∈[0, 1] and ∈(0, 1], which control the step
size and injected noise respectively. We chose h = 0.01, = 0.1, 0 = 1, and = 0.05.

Published as a conference paper at ICLR 2024
Algorithm 1 Sampling via ascent of the log-likelihood gradient from a denoiser residual
Require: denoiser f, step size h, stochasticity from injected noise , initial noise level 0, final
noise level , distribution mean m
1: t = 0
2: Draw x0 ∼N(m, 2
0Id)
3: while t do
...
difference between train and test PSNRs averaged across noise levels ∈[0, 1]. Each curve shows
...
Top. PSNR curves of trained networks for various regularity levels . The empirical slopes achieved
for different values of closely match the optimal slopes (dashed lines). Bottom. Eigenvectors for
two C images (top row: = 4, bottom row: = 2), which consist of harmonics on the two regions
...
Bottom. Top 10 basis vectors for each image are shown. With increasing 1, the contours become
...
normalized vector x/∥x∥with associated shrinkage factor = 1, whereas the remaining basis vectors
should have shrinkage factors of = 0 but are otherwise unconstrained. This optimal denoiser
achieves an MSE of 2, and thus a linear PSNR curve with unit slope and intercept 10 log10(d).
Top left. Denoising of the training image with = 0.04. Right. Decay of the coefficients ⟨x, ek⟩
and the shrinkage factors k. The DNN denoiser exhibits a slower decay of shrinkage factors than
...
curvature. For small , the manifold can be assumed to be locally flat, so that the optimal denoising
...
Clean, noisy ( = 0.08), and denoised test image. Middle left. The unit vectors spanning the
...
the images before display. Top left. Clean (shuffled then unshuffled), noisy (unshuffled, = 0.3),
and denoised (unshuffled) images. Middle. The shrinkage factors k(y) decay more slowly than
...
the score ∇log p(y) (dropping the dependence to simplify notation) and its Jacobian ∇2 log p(y)
...
∇y log p(y|x)
We then use the fact that y is obtained from x by adding Gaussian white noise of variance 2Id:
log p(y|x) = −1
22 ∥y −x∥2 + cst,
(18)
∇y log p(y|x) = −1
...
exploding SDE dxt = dwt where (wt)t0 is a Brownian motion (t = 2 then corresponds to the
...
as well as the expression of the score model sθ(y) = (fθ(y) −y)/2 into the score-matching error:
∥∇log p(y) −sθ(y)∥2i
= 1
4 E
∥E[x | y] −fθ(y)∥2i
...
∥y −x∥2i
(25)
The last term is the total variance of the noise and is thus equal to 2d. The middle term can be
rewritten with an integration by parts, using the fact that y −x = −2∇y log p(y|x):
E[⟨y −x, y −f(y)⟩] = −2
∇y log p(y|x), y −f(y)
...
+ 22E[tr ∇f(y)] −2d,
(27)
proving the Steins Unbiased Risk Estimator of the MSE.
...
k(x) ⟨y, ek⟩ek.
In practice, the denoiser does not have access to the clean image x, and the shrinkage factors k
...
(⟨x, ek⟩−k(x) ⟨y, ek⟩)2
= E
((1 −k(x))⟨x, ek⟩−k(x)⟨z, ek⟩)2
(1 −k(x))2⟨x, ek⟩2 + k(x)22
(28)
where the last step used the fact that ⟨z, ek⟩∼N(0, 2) independently from x. For each x and k, the
optimal oracle shrinkage factor k(x) thus minimizes the quadratic function
(1 −k(x))2⟨x, ek⟩2 + k(x)22,
...
⟨x, ek⟩2 + 2 .
(30)
Incidentally, this error is also equal to 2 P
k k(x), similarly to the optimal denoiser as shown in
eq. (10).
The fraction
2⟨x,ek⟩2
⟨x,ek⟩2+2 is of the same order as min(⟨x, ek⟩2, 2) up to a factor of 2, as we have the
...
⟨x, ek⟩2.
(31)
Let M be the number of terms in the left sum (that is, the number of ranks k such that ⟨x, ek⟩2 > 2),
and xM = P
⟨x,ek⟩2>2⟨x, ek⟩ek be the M-term approximation of x. We then have
∥x −xM∥2 =
...
corresponds to the signal variance that has been lost, and a denoising variance M2, which cor-
...
x), and assume that ⟨x, ek⟩2 ∼k−(+1) for some > 0. More precisely, we assume that there
exists two constants c, c′ independent of x and k such that c k−(+1) ⟨x, ek⟩2 c′ k−(+1). By
definition of M,
⟨x, eM⟩2 > 2 ⟨x, eM+1⟩2,
so that
c′ M −(+1) > 2 c (M + 1)−(+1).
We then have M −(+1) ∼2, i.e., M ∼−2/(+1), and thus M2 ∼2/(+1). We also have
k>M
⟨x, ek⟩2 c′ X
k>M
k−(+1) c′
...
t−(+1)dt = c
(M + 1)−,
so that ∥x −xM∥2 ∼M − ∼2/(+1). Finally, we have shown that the two terms in eq. (33) are
...
that the oracle MSE is ∼2/(+1).
GEOMETRIC C IMAGES
A continuous image x: [0, 1]2 R is part of the geometric C class (Korostelev & Tsybakov, 1993;
Donoho, 1999; Peyré & Mallat, 2008) if it is uniformly -Lipschitz over [0, 1]2 \ {i}, where the
i are uniformly -Lipschitz curves in [0, 1]2 which do not intersect tangentially. A function f is
uniformly -Lipschitz over a domain Ωif there exists a constant C such that for all x ∈Ω, there
exists a polynomial qx of degree ⌊⌋such that for all y ∈Ω,
|f(y) −qx(y)| C |x −y|.
(34)
We explain how to generate numerically such images in Algorithm 2.

Published as a conference paper at ICLR 2024
Algorithm 2 Synthesis of a C image via integration
Require: regularity , Fast Fourier Transform FFT
1: Make a contour
2: Define a 1D filter f1(ω) = |ω|−
