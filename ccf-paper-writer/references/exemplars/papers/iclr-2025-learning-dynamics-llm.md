...
ences the models predictions on other examples, gives us a powerful tool for understanding
...
sis not only provides a novel perspective of understanding LLMs finetuning but also inspires
...
This procedure can be described by learning dynamics, which links changes in the models predictions to the
...
pipeline, with differing explanations as to why they improve the models performance.
...
models prediction into three terms which play different roles. This framework can be easily adapted to
...
for each tokens prediction, the negative gradient will push down the models predictions on (almost) all
...
models prediction. In this paper, we narrow down it to describe how the change in models parameter θ
...
After an GD update on xu, how does the models prediction on xo change?
...
of each column. We can track the change in the models confidence by observing log πθ(y | x).
...
At(xo) = I −1π⊤
θt(xo) only depends on the models current predicted probability. The matrix Kt is the
empirical neural tangent kernel (eNTK, Jacot et al. 2018) of the model, the product of the models gradients
...
different input samples: larger ∥Kt∥F means the update of xu likely influences the models prediction on xo
more. Finally, Gt is determined by the loss function L, which provides the energy and direction for the models
adaptation. For example, for cross-entropy loss LCE ≜−y⊤
u log π(y | xu), we have Gt
CE = πθt(y | xu) −yu,
a length-V vector that points from the models current predictive distribution to the desired supervisory
...
changes the models prediction on xo for each learning step. Since a real model updates its parameters for
...
from πθt(y | xu) and point to e4. We can then ask how the models predictions on different xo change after
...
xus class (not the correct label of xo) would be pulled up, as in the second panel of Figure 1-(b). Last, for
examples that look dissimilar to xu (small ∥Kt∥F ), this update will not change the models prediction on xo
...
Published as a conference paper at ICLR 2025
LEARNING DYNAMICS OF LLMS FINETUNING
...
input and output signals. The high-dimensional property makes it hard to observe the models output, and the
...
the models prediction on the m-th token of yo can be represented as, when z gradients have bounded norm,
[∆log πt(y | χo)
V M
]m = −
l=1
...
u influence the models belief about a response y′
When tracking the models confidence on different responses given the question xu, learning from y+
u will
...
u ) −y−
(7)
where a is the margin (i.e., the (·) value) for the l-th token, which represents how well the current policy
separates y+
u and y−
u compared with the reference policy. Due to the monotonicity of (·), a larger margin
leads to larger a, which in turn restrains the strength of Gt
DPO+/-. In other words, Gt
DPO+/- automatically provides
less energy on the examples that are already well separated. We then check the role of , which controls the
regularizing effect on the KL distance between πθt and πref in the original RL loss (Rafailov et al. 2023).
When the margin is negative, larger leads to a smaller a and hence provides stronger Gt
DPO+/- for the model
to catch up the separating ability of the reference model faster. But when the model is better and has a
positive margin, increasing will increase a and hence create a negative influence on (1 −a), which makes
...
responses come from the less likely region of the models prediction. On-policy responses, though, are more
...
label y−
u , we can describe the changing of models predictive distribution πθt+1 as follows:
...
is common for a pretrained model, all πθt+1(y ̸= y) decrease (only yis rich).
Trend: smaller πθt(y−
...
both y+
u and y−
u are not sampled from the models distribution, ysometimes can be dissimilar to y+
u , and the
u are likely located in a valley region of the models prediction. Then its learning dynamics would look like
...
dynamics, which can not only support our analysis above, but also shed more light on how the models
...
u , because the
models predicted probability to all responses in Y-space must sum to one. The models behavior on different
y is mostly a trade-off among these pressures. To verify this claim, we finetune the model for several epochs
and evaluate the models prediction on all responses in Dprob every 25 updates (with a training batch size of 4,
the probing occurs every 100 examples). For each type of response, we average the models confidence on all
500 examples and report the mean value of their log-likelihood.
As demonstrated in the first panel of Figure 3, the models confidence on y+
...
from the test set (y+
test), or even a randomly generated English sentence (yhum). The models confidence in
...
u will indirectly increase the models confidence of these similar y. That is why the
...
set. For these responses, the models prediction on πθt(y+
...
Last, to further explore the similarity between different responses from the models perspective. we SFT
...
To verify our framework also explains the models behavior in preference tuning, we conduct similar
...
models performance during DPO, we can first train the model on both [xu; y+
u ] and [xu; y−
u ] during the SFT
...
efficiently decrease the models confidence on y−
...
Learning dynamics, which depict how the models prediction changes when it learns new examples, provide a
...
LLMs instruction tuning and preference tuning, some of them are quite counter-intuitive. We also shed
...
preferences. International Conference on Artificial Intelligence and Statistics. PMLR, pages 44474455.
...
suite for analyzing large language models across training and scaling. ICML. PMLR, pages 23972430.
...
document recognition. Proceedings of the IEEE 86.11, pages 22782324.
...
tions with human feedback. NeurIPS, pages 2773027744.
...
Yulong Chen, et al. (2023). Sirens song in the AI ocean: a survey on hallucination in large language
...
Instead of emphasizing the models convergence status and guarantees, learning dynamics focus more on its
...
why successive knowledge transferring can improve the models systematic generalization ability.
Besides explaining the models behavior, learning dynamics is also helpful for evaluating the quality or the
...
use y+
j̸=i when answering question i. Holtzman et al. (2020) related the peakiness of the models distribution
to LLMs repeater phenomena, which also indirectly supports our claims well: more DPO leads to a more
serious squeezing effect, hence the models prediction becomes peakier on most tokens, which makes the
...
careful when working on the valley region of the models distribution. Other works try to analyze this
problem by inspecting the token-level influence between responses. For example, Pal et al. (2024) assumes
u and y−
u are identical expect one token. Under this assumption, the models confidence of y+
u after the
identical token is guaranteed to decrease. They propose a solution by significantly enhancing the learning rate
(roughly x50 larger when their = 50) of the positive part when detecting yu located in a low-confidence
...
On the other hand, the effect that negative gradients make the models distribution peakier is independently
...
Published as a conference paper at ICLR 2025
Proof. 2 Suppose we want to observe the models prediction on an observing example xo. Starting from
...
= O
 η2∥∇θz(xu)∥2
In the decomposition, using {π1, . . . , πV } to represent the models prediction on different dimensions, we
can write our At as:
At(xo) = I −1(πt)⊤=
1 −π1
1 −π2
...
the networks notion of similarity evolves. For appropriately initialized very wide networks trained with
...
Kt(xo, xu) is unchanged (usually used in theoretical analysis) might be too strong in the LLMs finetuning.
...
different l is independently generated as long as the shared network is fixed, the yl for the LLMs output
depends on y<l, which is usually sampled from the models prediction iteratively. However, in most of
...
Kt(χo, χu). After this step, the decomposition of LLMs finetuning learning dynamics then becomes similar
to a multi-label classification task.
B.2.1
INSTRUCTION FINETUNING USING AUTO-REGRESSION LOSS (SFT)
Here we derive the residual term, i.e., Gt for different algorithms in LLMs finetuning. We first rewrite
Equation (5) here:
[∆log πt(y | χo)
V M
]m = −
...
usually correlated with the confidence gap between the models current confidence on y+
u and y−
u , i.e.,
...
Similarly, we can also see a hard version of the regularization effect mentioned above. If Gap(πθt) > , the
...
can figure out the difference between the models prediction of those directly influenced responses (y appears
...
Prompt: 𝐱
0. Initial response
Im not a chemist, but Im guessing that most of the cigarette odor comes from the
...
Lets take a look at the next months release schedule. Are there any games youre particularly
excited for?
4. Irrelavent from test set
Youve searched through quite a few results and havent come across a recipe you like yet.
5. Random sentence
...
7. Pure random tokens
you a through few Youve recipe yet. and across quite a searched come havent results like
Im not sure what you mean. Im not sure what youre asking.
...
hold in these experiments. We then compare the models behavior in these two phases respectively. In the
...
models prediction, will push down the whole curve significantly, except the one with the highest confidence
...
can be verified by comparing the curves of the last two panels: longer SFT makes the models prediction
...
u in LLMs finetuning. As also pointed by Razin
...
to a length-d feature vector via a deep neural network ϕ. In other words, we have ϕ(x) ∈Rd1. The model
uses a linear read-out layer w ∈RdV to convert the feature vector to logits z = w⊤ϕ(x) and then generate
...
quantitatively analyze how the models confidence in each class changes, we define a ratio i ≜pt+1
and use
...
Published as a conference paper at ICLR 2025
Note that the values of j also depends on whether i equals y, hence for Case 1 (i = y) and Case 2 (i ̸= y),
we have (η′ ≜η∥ϕ(x)∥2
...
ezt
j=1 e
j , we can derive i and j as the left part of Equation (35). Similarly,
when i ̸= y, we have:
pt+1
i̸=y =
ezt+1
...
We can now better understand how each pi changes after this update. Specifically, if i > 1, the corresponding
pi increases, and vice versa. To determine the value of i, we can treat any j > 1 as contributing to the
...
Published as a conference paper at ICLR 2025
Claim 1: The value of py is guaranteed to decrease, i.e., y < 1.
We start from the value of in Case
1 as illustrated in Equation (35). It is clear that for any j ̸= y, we have j > 1, because 1 + pt
j −pt
i > 0.
Combining with y = 1, it is straightforward to have Claim 1.
Claim 2: The value of piwhere i= argmaxi∈[V ]\{y} pt
i is guaranteed to increase, i.e., i> 1.
now use the value of in Case 2, since icannot equal y by definition. When j ̸= y, we have pt
j −pt
for all possible j, because pt
iis the largest among all pt
i̸=y of pt. Hence all j̸=y must be smaller than one.
Combining with the fact that y < 1 (because pt
y −pt
i−1 must be negative), we can prove that i> 1.
...
into other pi (i.e., other pi increases)? Or will piabsorb the mass not only from py but also from other
...
Equation (34) can be simplified to i =
j=1 j . Note that the first two claims hold for any distribution
pt, hence we only check the values of i̸=y here to verify the uniformly distributed mass hypothesis.
Substituting the values of j to this new leads to i =
V −1+eη′ for all i ̸= y. Since η′ < 0 and eη′ < 1,
we must have i̸=y > 1. Combined with the fact that all pt
...
understand the influence of pt better. Assume we are observing i′ where i′ is not y nor i. We consider two
...
j∈B > 1.
(40)
Note that we misuse the ≪notation to highlight the fact that j=y would be much smaller than j∈S, because
there is a negative one term in the exponential. With the above expression, we can imagine that if pt
i′ is
relatively small, the size of B would be large, which means there will be more j > 1 contributing to the
conclusion that i′ < 1. If the influence of j∈B is strong enough to override the influence of other
(especially j=y which is way smaller than other ), i′ would be smaller than one and hence pi′ decreases.
On the contrary, for those i′ with relatively large pt
i′, the < 1 terms becomes dominant and hence lead to
i′ > 1, i.e., pi′ increases.
In the analysis above, we assume pt is only slightly non-uniform (i.e., not so peaky), which means the
values of different ezt
j are relatively comparable. However, in practical machine learning systems like LLMs
finetuning, the distribution pt would be very non-uniform, which means most of the probability mass is
obtained by a few dimensions. That is because the LLMs vocabulary size is usually very large and the
...
ithat dominates . In other words, i
ezt
i. Then for any i′ we want to
observe, the i′
i< 1. In other words, the models predictions on all dimensions other than the one
...
illustrated in Equation (40), where the value of is decomposed into three subgroups. Recall the definition of
i, we know all j < 1 contribute to the hypothesis that pi increases after this update, where the strength
of this contribution is controlled by ezt
j. Since a pt
y small means a small ezt
j, the influence of j=y ≪1 is
significantly weakened under this scenario. In other words, i < 1 is more likely to occur for all possible i,
...
learning rate η′ < 0 is a shared scalar in all j. Hence larger |η′| can amplify all the trends aforementioned.
Furthermore, recall the shape of an exponential function ex, where a small change of x (especially when x > 1)
will make ex changes a lot. Then the terms j̸=y = e−η′(1+pt
j−pt
i) in Case 1 and j=y = e−η′(pt
j−pt
i−1) in
...
demonstrate the models predicted distribution before and after this update. As we expected, the positive
...
we assume the models prediction on x is relatively flat, as demonstrated in the second row of Figure 20,
...
Figure 19.
Although the behavior described above follows our intuitions well, a flat p0 is not common in LLMs
finetuning. Because finetuning usually starts from a pre-trained w, where the models prediction would likely
...
use η = −0.5 to push down the models prediction on this class. As demonstrated by the blue curve,
...
u are sampled from
the high confidence region of the models predictions. Then, what will happen if we conduct off-policy
...
is also quite consistent as long as we impose a big negative gradient on the valley region of the models
...
consistent trends in LLMs finetuning experiments. First, by comparing the average decaying speed of the
...
models prediction. Such a change in turn weakens the squeezing effect, as illustrated in Figure 5.
...
Published as a conference paper at ICLR 2025
that these methods improve the models performance not only by mitigating the squeezing effect. Hence
to figure out to what extent the squeezing effect can harm the models performance, we propose a simple
...
can pull up the region of supervised ˆy and we dont want the model to impose big negative gradients on a
...
u and y−
u well. In other words, under DPOs
