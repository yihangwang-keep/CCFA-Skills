...
sis not only provides a novel perspective of understanding LLM’s finetuning but also inspires
...
This procedure can be described by learning dynamics, which links changes in the model’s predictions to the
gradients generated by learning specific examples. With the help of learning dynamics, researchers have not
only explained many interesting phenomena during training, e.g., the “zig-zag” learning path (Ren et al. 2022)
...
pipeline, with differing explanations as to why they improve the model’s performance.
...
et al. 2024; Tajwar et al. 2024), this paper tries to understand LLMs’ evolution from a dynamical perspective.
Specifically, we formalize the learning dynamics of LLM finetuning by decomposing the change of the
model’s prediction into three terms which play different roles. This framework can be easily adapted to
1
...
counter-intuitive observations during training – including the “repeater” phenomenon after preference tuning
...
observing an interesting “squeezing effect,” which we demonstrate is a consequence of gradient ascent (as
in DPO and similar algorithms) on models with cross-entropy loss following a softmax layer. In short,
for each token’s prediction, the negative gradient will push down the model’s predictions on (almost) all
possible output labels, moving this probability mass to the most-likely labels. This can be detrimental to the
alignment we are trying to achieve. This effect is most serious when the negative gradient is imposed on
an already-unlikely label, which is why the confidence of almost all responses decreases during off-policy
DPO. Inspired by this, we propose a simple, counter-intuitive, but very effective method to further improve
alignment performance.
2
DEFINITION OF LEARNING DYNAMICS AND AN MNIST EXAMPLE
Learning dynamics is usually an umbrella term describing how the change of a specific factor influences the
model’s prediction. In this paper, we narrow down it to describe “how the change in model’s parameter θ
influences the corresponding change in fθ”, i.e., the relationship between ∆θ and ∆fθ. When the model
updates its parameters using gradient descent (GD), we have
∆θ ≜θt+1 −θt = −η · ∇L (fθ(xu), yu) ;
∆f(xo) ≜fθt+1(xo) −fθt(xo),
(1)
where the update of θ during step t →t + 1 is given by one gradient update on the sample pair (xu, yu) with
learning rate η. In short, the learning dynamics in this paper address the question:
After an GD update on xu, how does the model’s prediction on xo change?
Studying the learning dynamics can shed light on many important problems in deep learning and also help to
understand various counter-intuitive phenomena. Appendix A further discusses related work.
As a warm-up, we first consider a standard supervised learning problem, where the model learns to map x
to predictions y = {y1, . . . , yL} ∈VL, where V is the vocabulary of size V . The model usually outputs a
probability distribution by first generating a matrix of logits z = hθ(x) ∈RV ×L and then takes the Softmax
of each column. We can track the change in the model’s confidence by observing log πθ(y | x).
Per-step influence decomposition.
The learning dynamics of (1) become,
∆log πt(y | xo) ≜log πθt+1(y | xo) −log πθt(y | xo), .
(2)
For simplicity, we start from the L = 1 scenario, where the ∆θ and ∆log π can be linked by the following
result, a version of a result of Ren et al. (2022) proved and further discussed in Appendix B. For multi-label
classification (L > 1), the updates separate; we can calculate L different ∆log πt and stack them together.
Proposition 1. Let π = Softmax(z) and z = hθ(x). The one-step learning dynamics decompose as
∆log πt(y | xo)
|
{z
}
...
op),
(3)
where At(xo) = ∇z log πθt(xo) = I −1π⊤
θt(xo), Kt(xo, xu) = (∇θz(xo)|θt)(∇θz(xu)|θt)⊤is the empirical
neural tangent kernel of the logit network z, and Gt(xu, yu) = ∇zL(xu, yu)|zt.
2
...
(𝐱𝐮=
, 𝐲𝐮= 4)
(a) Adaptation vector created by (𝐱𝐮, 𝐲𝐮)
(b) One-step change with the
same 𝒢𝑡(𝐱𝐮, 𝐲𝐮) (large 𝜂)
(c) Accumulated change of epochs
Δ log 𝜋𝑡(𝐱𝐨) = −𝜂𝒜𝑡(𝐱𝐨) 𝒦𝑡(𝐱𝐨, 𝐱𝐮) 𝒢𝑡(𝐱𝐮, 𝐲𝐮)
Strong influence
, 𝐲
(
, 𝐲)
...
At(xo) = I −1π⊤
θt(xo) only depends on the model’s current predicted probability. The matrix Kt is the
empirical neural tangent kernel (eNTK, Jacot et al. 2018) of the model, the product of the model’s gradients
...
The common “lazy eNTK” assumption discussed in Arora et al. (2019) is a sufficient but not necessary
...
different input samples: larger ∥Kt∥F means the update of xu likely influences the model’s prediction on xo
more. Finally, Gt is determined by the loss function L, which provides the energy and direction for the model’s
adaptation. For example, for cross-entropy loss LCE ≜−y⊤
u log π(y | xu), we have Gt
CE = πθt(y | xu) −yu,
a length-V vector that points from the model’s current predictive distribution to the desired supervisory
distribution. For typical “hard” labels, yu is a one-hot vector eyu.
Accumulated influence and a demonstration on MNIST.
Proposition 1 describes how the update of xu
changes the model’s prediction on xo for each learning step. Since a real model updates its parameters for
...
See Figure 1-(a), where the network πθt is updating its parameters using the loss calculated on one training
example (xu, yu = e4). The residual term Gt
CE(xu, yu) is then represented by the red arrows, which all start
from πθt(y | xu) and point to e4. We can then ask how the model’s predictions on different xo change after
this update. As in Figure 1-(b), for an xo in the same class with xu (i.e., the identical case), the predicted
probability of this correct label is “pulled up” by this update, as expected. On the other hand, if this xo is
similar to xu (i.e., ∥Kt∥F is reasonably large) but comes from another class, then the predicted probability on
xu’s class (not the correct label of xo) would be “pulled up,” as in the second panel of Figure 1-(b). Last, for
examples that look dissimilar to xu (small ∥Kt∥F ), this update will not change the model’s prediction on xo
...
To verify this, we plot the average of π(y | x) for x from each of the classes in Figure 1-(d). The values of
...
input and output signals. The high-dimensional property makes it hard to observe the model’s output, and the
...
are many different algorithms for LLM finetuning – SFT (Wei et al. 2022), RLHF (Ouyang et al. 2022), DPO
(Rafailov et al. 2023), etc. – analyzing them under a uniform framework is challenging. Finally, compared
with the training-from-scratch scenario, where a roughly uniform distribution over all possible outputs is
usually assumed, LLMs’ finetuning dynamics heavily rely on the pretrained base model, which could make
the analysis harder. For example, the pretrained model usually assigns little probability mass to unlikely
tokens, which is good for most applications but leads to risk of the “squeezing effect” we show later. We now
tackle these problems and propose a unified framework for different finetuning methods.
3.1
PER-STEP DECOMPOSITION OF THE SFT LOSS
The typical loss function used during supervised finetuning is the negative log-likelihood (NLL) of a given
completion y+
u = [y+
1 , . . . , y+
L ] ∈VL, conditioned on the prompt xu:
LSFT(xu, y+
u ) ≜−
L
X
...
of all labels can be factorized as π(y | x) = Q
l π(yl | x), the sequential nature of language modeling
makes the analysis more complicated, because we must have π(y | x) = Q
l π(yl | x, y<l). To solve this
problem, we can merge this factorization into the definition of the backbone hθ while keeping the format of
Proposition 1. Specifically, letting χ be the concatenation of x and y, the prediction of all tokens of y is
z = hθ (χ) ;
π (y | χ) = Softmax_column (z) .
Here z is a V ×L matrix where each column contains the logits of the prediction of the lth token. Our hθ, even
though it takes the entire sequence χ as its input, will force the model not to refer to the future tokens y>l when
making predictions on the l-th token, commonly implemented via “causal masking” (proposed in Vaswani
et al. (2017), details in Figure 10a of Appendix D). Then, we can calculate (∇θzl(χo)|θt)(∇θzl(χu)|θt)⊤
on each column of z and stack them to form a V × V × L tensor Kt(χo, χu). The calculation of Gt and At
also follows a similar procedure. Thanks to the causal mask implemented in hθ, the resulting decomposition
is almost identical to that in a multi-label classification problem. Assuming have a response yu of length L
associated with xu, stacked into χu, and yo of length M associated with xo, stacked into χo. The change of
the model’s prediction on the m-th token of yo can be represented as, when z gradients have bounded norm,
[∆log πt(y | χo)
|
{z
}
...
(5)
where Gt
SFT (χu) = πθt(y | χu) −yu. Compared with Proposition 1, the main difference is that the eNTK
...
u influence the model’s belief about a response y′
u?
When tracking the model’s confidence on different responses given the question xu, learning from y+
u will
impose a strong “upwards” pressure on y+
u , as illustrated in the first panel of Figure 2. At the same time,
4

Published as a conference paper at ICLR 2025
the confidence of “similar responses” will also be slightly pulled up, like how learning a 4 influences the
prediction on 9 in the MNIST example. We will discuss how to understand the “similarity” between two
sequences of responses in the next section.
3.2
PER-STEP DECOMPOSITION OF THE DPO LOSS
...
training tells the model “what to do”, many preference finetuning methods also teach the model “what not
to do,” which makes the learning dynamics more complex. For intuition, we start by analyzing a typical
...
where y+
u and y−
u are pre-generated responses, and πref is the reference model, typically the result of SFT.
In the loss function, the πθt terms are also calculated using teacher forcing. Hence we can decompose the
learning dynamics for DPO similarly to Equation (5),
[∆log πt(y | χo)]m = −
L
X
l=1
...
,
(7)
where a is the margin (i.e., the σ(·) value) for the l-th token, which represents how well the current policy
separates y+
u and y−
u compared with the reference policy. Due to the monotonicity of σ(·), a larger margin
leads to larger a, which in turn restrains the strength of Gt
DPO+/-. In other words, Gt
DPO+/- automatically provides
less energy on the examples that are already well separated. We then check the role of β, which controls the
regularizing effect on the KL distance between πθt and πref in the original RL loss (Rafailov et al. 2023).
When the margin is negative, larger β leads to a smaller a and hence provides stronger Gt
DPO+/- for the model
to “catch up” the separating ability of the reference model faster. But when the model is better and has a
positive margin, increasing β will increase a and hence create a negative influence on β(1 −a), which makes
the model update less. This aligns well with the claims of Rafailov et al. (2023): the stronger regularizing
effect tends to “drag πθ back towards πref” when its predictions deviate from πref too much. The derivation
...
responses come from the “less likely” region of the model’s prediction. On-policy responses, though, are more
...
3.3
THE SQUEEZING EFFECT CAUSED BY NEGATIVE GRADIENT
...
responses sampled from π in an on-policy way. In the second panel, we demonstrate the “squeezing effect”
caused by imposing a big negative gradient on a “valley” region of a distribution. For more details about this
...
trend becomes more serious if πθ0 is finetuned for longer before conducting DPO (reported in Figure 3 of
Rafailov et al. (2024) and verified by our Figure 17). Furthermore, we also find that for all the πθt(y | χu)
...
label y−
u , we can describe the changing of model’s predictive distribution πθt+1 as follows:
• Guarantee: the confidence of y−
u , i.e., πθt+1(y−
u ) will decrease.
• Guarantee: the decreased probability mass is largely “squeezed” into the output which was most
confident before the update: if y∗= argmaxi∈[V ]\{y−
u } πθt(y = i), then πθt+1(y = y∗) will increase.
• Trend: the rich get richer and the poor get poorer: generally, dimensions with high πθt tend to increase,
and those with low πθt tend to decrease.
• Trend: peakier πθt squeezes more. If the probability mass concentrates on few dimensions in πθt, which
is common for a pretrained model, all πθt+1(y ̸= y∗) decrease (only y∗is “rich”).
• Trend: smaller πθt(y−
u ) exacerbate the squeezing effect: if y−
u is unlikely under πθt, the probability mass
of all other πθt+1(y ̸= y∗) will be more seriously decreased, and the πθt+1(y = y∗) increases more.
Appendix E illustrates the squeezing effect and analytically proves its existence for logistic regression models,
by directly computing πθt+1/πθt in different situations. Section 4.2 also experimentally verifies the analysis
...
both y+
u and y−
u are not sampled from the model’s distribution, y∗sometimes can be dissimilar to y+
u , and the
y−
u are likely located in a valley region of the model’s prediction. Then its learning dynamics would look like
the sketch in the second panel of Figure 2: the confidence on almost all y are pushed down. At the same time,
6

Published as a conference paper at ICLR 2025
all the decreased probability mass is squeezed to y∗, which might make the model keep generating repeated
phrases, as reported by Holtzman et al. (2020). Variants of DPO algorithms often unintentionally mitigate this
squeezing effect by constraining the strength of the negative gradient or the positions of y−
u , which partially
...
response y−. SFT finetunes with x and y+, while DPO uses all three (subscripts of x and y are removed
...
corresponding x, y+, or y−. (We also study another probing dataset where all x come from the test set in
an ablation study in the appendix.) Then for each x in Dprob, we can observe how log πθt(y | χ) gradually
changes on different types of y. For example, one extended response type can be a rephrase of y+, an
irrelevant response answering another question x′, or just a randomly generated English sentence with the
same number of words with y+. We explain why we need these extended responses and how they are
generated in detail in Appendix D.1. In short, Dprob helps us to get a more fine-grind inspection of the learning
dynamics, which can not only support our analysis above, but also shed more light on how the model’s
prediction evolves on the entire Y ∈RV ×L, a very sparse and huge space.
4.1
...
model’s confidence on y+
u , but also indirectly “pulls up” responses similar to y+
u with a smaller strength
(scaled roughly by ∥Kt∥F ), similar to how learning a “4” influences the prediction of a “9” in the MNIST
example. At the same time, the increase of πθt(y+
u |χu) naturally “pushes down” all y ̸= y+
u , because the
model’s predicted probability to all responses in Y-space must sum to one. The model’s behavior on different
y is mostly a trade-off among these pressures. To verify this claim, we finetune the model for several epochs
and evaluate the model’s prediction on all responses in Dprob every 25 updates (with a training batch size of 4,
the probing occurs every 100 examples). For each type of response, we average the model’s confidence on all
500 examples and report the mean value of their log-likelihood.
As demonstrated in the first panel of Figure 3, the model’s confidence on y+
u keeps increasing throughout the
whole learning process, which is straightforward because the main “pull-up” pressure is imposed directly on
y+
u . However, the behavior of some responses similar to y+
u is non-trivial. For example, we draw the following
types of responses in the same panel, i.e., the less preferred response for the same question (y−
u ), two types
of rephrases of y+
...
from the test set (y+
test), or even a randomly generated English sentence (yhum). The model’s confidence in
...
be well explained by the learning dynamics we discussed before. Since all these examples are “similar” to
y+
u to some extent (at least, they are all common “standard English” sentences), their ∥Kt∥F are reasonably
large. Then learning y+
u will indirectly increase the model’s confidence of these similar y. That is why the
corresponding πθt(y|χu) are slightly increased at the beginning of training. However, as the training goes
7
...
150
140
130Whether y occured during training
y +
j
u
y +
...
on, the model’s confidence on y+
u keeps increasing and the update energy, the norm of Gt
SFT in Equation (5),
gradually decreases. That means the indirect “pull-up” pressures are also diminished accordingly. Then, the
“push-down” pressure on all y ̸= y+
u becomes dominant and all the related curves start going down.
To verify the existence of this global “push-down” pressure, we observe two types of responses; both have the
same number of words as their y+
u . One is a purely random English word sequence y′
rnd. Another is a random
permutation of all the words in y+
u , which is called y+
urnd. Since both are not natural language, we expect the
∥Kt∥F between them and y+
u to be very small, which means learning from y+
u imposes almost no “pull-up”
pressure on them; thus the “push-down” pressure will dominate through the whole training procedure. These
analyses are well supported by the second panel in Figure 3, in which we see these πθt(y|χu) all start from a
very small value, and keep decreasing throughout the training.
Another interesting type of responses is y+
j̸=u, a preferred response for another question xj̸=u in the training
set. For these responses, the model’s prediction on πθt(y+
j̸=u|χu) will be kept influenced by two “pull-up”
pressures: one is from learning [xu; y+
u ], another is from learning [xj̸=u; y+
j̸=u], where the latter might be even
stronger as the gradient is directly calculated by observing y+
j̸=u. That explains why we see the confidence
on y+
j̸=u keeps increasing with a smaller rate compared with y+
u in the third panel. Because the “pull-up”
pressure is always strong enough to counter the “push-down” one. These observations provide us with a
unique explanation of why specific types of hallucinations are amplified after SFT. Specifically, the increase
of πθt(y+
j̸=u|χu) means if we ask the model to answer a question xu, it might provide a response from (or
partially from) another unrelated question xj̸=u in the training set.
Last, to further explore the “similarity” between different responses from the model’s perspective. we SFT
the model using more types of responses and observe how πθ(y′ | χu) changes accordingly. The results are
...
first column resembles how different [xu; y′] changes when we SFT the model using [xu; y+
u ]. One interesting
...
considered as a type of “fingerprint”. We left this interesting problem for our future work.
4.2
LEARNING DYNAMICS OF OFF-POLICY DPO
To verify our framework also explains the model’s behavior in preference tuning, we conduct similar
...
u consistently decay slower than those of y−
u ,
although none of them ever occur during training. That is because these responses are close to y+
u or y−
u in
Y, which means their ∥Kt∥F is relatively large. Hence the pressures imposed on y+
u and y−
u also introduce
a non-negligible influence on them. Last, in the fourth panel, the margin πθt(y+
u |χu) −πθt(y−
u |χu) keeps
...
Although Gt
DPO directly imposes a “pull-up” pressure on y+
u , the value of πθt(y+
u |χu) does not increase a lot
as it does in SFT. The downward arrow on y−
u indeed introduces a “push-down” pressure on responses that
are similar to y−
u , but the influence is unlikely to be that strong (it will be weakened by ∥Kt∥F ) to make the
confidence on almost every observing responses decrease so fast, as demonstrated in the last panel of Figure 3
where we use similar η for both SFT and DPO. Then, where has the probability mass gone during DPO? The
key to answering this question is the squeezing effect discussed in Section 3.3: since the big negative gradient
is imposed on y−
u , which is at this point probably in a region of low πθt(y|χu), the confidence of most y will
be decreased and πθt(y∗|χu) will increase very fast.
...
u . As
illustrated by the last panel of Figure 4, the confidence of this “teacher forcing” greedy y increases very fast
(from -113 to -63), which is even faster than the increase of πθt(y+
u |χu) during SFT (from -130 to -90), within
8 epochs. However, the tokens with the highest confidence do not necessarily form a preferred response: it
will reinforce the prior bias in θ0. This could be a reasonable explanation of the “degeneration” reported in
recent work (e.g. Holtzman et al. 2020): as πθt becomes more peaky at its most confident predictions, it is
...
4.3
MITIGATING THE SQUEEZING EFFECT BY AUGMENTING THE TRAINING SET FOR SFT
Since the “squeezing effect” caused by the big negative gradient on unlikely predictions can damage the
model’s performance during DPO, we can first train the model on both [xu; y+
u ] and [xu; y−
u ] during the SFT
...
responses compared with the baseline SFT. After that, the “push-down” pressure imposed during DPO can
9
...
u during SFT; 2.) The
extend method considerably increase y−
u during SFT; 3.) The squeezing effect of the extend method is weaker
(all other responses decay slower and the confidence on the “greedy-decoding” response increases slower).
efficiently decrease the model’s confidence on y−
u and its similar responses. Since y−
...
0.6667
0.5432
We call our training pipeline “extend” and compare its learning dynam-
...
policy network πbase and πextend. Then, we conduct identical DPO
training on both πbase and πextend for several epochs. The win rate of the proposed method against the baseline
one is provided in Table 1. It is clear that before DPO, πbase is better, because πextend is explicitly trained
on those y−. However, the πextend performs better after DPO several epochs since the squeezing effect is
...
Learning dynamics, which depict how the model’s prediction changes when it learns new examples, provide a
...
common algorithms. Then, we propose a unified framework for understanding LLM predictions’ behaviors
across different finetuning methods. The proposed analysis successfully explains various phenomena during
LLM’s instruction tuning and preference tuning, some of them are quite counter-intuitive. We also shed
...
REFERENCES
Sanjeev Arora, Simon S. Du, Wei Hu, Zhiyuan Li, Ruslan Salakhutdinov, and Ruosong Wang (2019). “On
Exact Computation with an Infinitely Wide Neural Net.” NeurIPS. arXiv: 1904.11955.
...
preferences.” International Conference on Artificial Intelligence and Statistics. PMLR, pages 4447–4455.
Jinze Bai, Shuai Bai, and et.al (2023). “Qwen Technical Report.” arXiv: 2309.16609.
Yuntao Bai, Andy Jones, Kamal Ndousse, Amanda Askell, Anna Chen, Nova DasSarma, Dawn Drain,
Stanislav Fort, Deep Ganguli, Tom Henighan, et al. (2022). “Training a helpful and harmless assistant with
...
Mohammad Aflah Khan, Shivanshu Purohit, USVSN Sai Prashanth, Edward Raff, et al. (2023). “Pythia: A
suite for analyzing large language models across training and scaling.” ICML. PMLR, pages 2397–2430.
...
converts weak language models to strong language models.” arXiv: 2401.01335.
Leshem Choshen, Lior Fox, Zohar Aizenbud, and Omri Abend (2020). “On the Weaknesses of Reinforcement
Learning for Neural Machine Translation.” International Conference on Learning Representations.
...
(2024). “Does Fine-Tuning LLMs on New Knowledge Encourage Hallucinations?” arXiv: 2405.05904.
...
Shangmin Guo, Yi Ren, Stefano V Albrecht, and Kenny Smith (2024). “lpNTK: Better Generalisation with
Less Data via Sample Interaction During Learning.” ICLR.
11

Published as a conference paper at ICLR 2025
Shangmin Guo, Biao Zhang, Tianlin Liu, Tianqi Liu, Misha Khalman, Felipe Llinares, Alexandre Rame,
Thomas Mesnard, Yao Zhao, Bilal Piot, et al. (2024). “Direct language model alignment from online AI
feedback.” arXiv: 2402.04792.
...
Weihua Peng, Xiaocheng Feng, Bing Qin, et al. (2023). “A survey on hallucination in large language
models: Principles, taxonomy, challenges, and open questions.” arXiv: 2311.05232.
Arthur Jacot, Franck Gabriel, and Clément Hongler (2018). “Neural tangent kernel: Convergence and
...
Tuning can Distort Pretrained Features and Underperform Out-of-Distribution.” ICLR.
Yann LeCun, Léon Bottou, Yoshua Bengio, and Patrick Haffner (1998). “Gradient-based learning applied to
document recognition.” Proceedings of the IEEE 86.11, pages 2278–2324.
Sheng Liu, Jonathan Niles-Weed, Narges Razavian, and Carlos Fernandez-Granda (2020). “Early-learning
regularization prevents memorization of noisy labels.” NeurIPS.
Mohamad Amin Mohamadi, Wonho Bae, and Danica J. Sutherland (2022). “Making Look-Ahead Active
Learning Strategies Feasible with Neural Tangent Kernels.” NeurIPS. arXiv: 2206.12569.
Long Ouyang, Jeffrey Wu, Xu Jiang, Diogo Almeida, Carroll Wainwright, Pamela Mishkin, Chong Zhang,
Sandhini Agarwal, Katarina Slama, Alex Ray, et al. (2022). “Training language models to follow instruc-
tions with human feedback.” NeurIPS, pages 27730–27744.
...
“Emergence of Hidden Capabilities: Exploring Learning Dynamics in Concept Space.” The Thirty-eighth
Annual Conference on Neural Information Processing Systems.
Garima Pruthi, Frederick Liu, Satyen Kale, and Mukund Sundararajan (2020). “Estimating training data
influence by tracing gradient descent.” NeurIPS.
Rafael Rafailov, Joey Hejna, Ryan Park, and Chelsea Finn (2024). “From r to Q∗: Your Language Model is
...
“Unintentional Unalignment: Likelihood Displacement in Direct Preference Optimization.” The Thirteenth
International Conference on Learning Representations.
Yi Ren, Shangmin Guo, Wonho Bae, and Danica J. Sutherland (2023). “How to prepare your task head for
finetuning.” ICLR.
Yi Ren, Shangmin Guo, Matthieu Labeau, Shay B. Cohen, and Simon Kirby (2020). “Compositional languages
emerge in a neural iterated learning model.” ICLR.
Yi Ren, Shangmin Guo, Linlu Qiu, Bailin Wang, and Danica J Sutherland (2024). “Bias Amplification in
Language Model Evolution: An Iterated Learning Perspective.” NeurIPS.
Yi Ren, Shangmin Guo, and Danica J. Sutherland (2022). “Better Supervisory Signals by Observing Learning
Paths.” ICLR.
Yi Ren, Samuel Lavoie, Michael Galkin, Danica J Sutherland, and Aaron C Courville (2023). “Improving
compositional generalization using iterated learning and simplicial embeddings.” NeurIPS.
Yi Ren and Danica J. Sutherland (2024). “Understanding Simplicity Bias towards Compositional Mappings
via Learning Dynamics.” NeurIPS 2024 Workshop on Compositional Learning: Perspectives, Methods,
and Paths Forward.
Rylan Schaeffer, Brando Miranda, and Sanmi Koyejo (2024). “Are emergent abilities of large language
models a mirage?” Advances in Neural Information Processing Systems 36.
...
Chelsea Finn, and Aviral Kumar (2024). “Preference Fine-Tuning of LLMs Should Leverage Suboptimal,
On-Policy Data.” arXiv: 2404.14367 [cs.LG].
Yunhao Tang, Daniel Zhaohan Guo, Zeyu Zheng, Daniele Calandriello, Yuan Cao, Eugene Tarassov, Rémi
Munos, Bernardo Ávila Pires, Michal Valko, Yong Cheng, et al. (2024). “Understanding the performance
gap between online and offline alignment algorithms.” arXiv preprint arXiv:2405.08448.
Ashish Vaswani, Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N. Gomez, Lukasz Kaiser,
and Illia Polosukhin (2017). “Attention Is All You Need.” NeurIPS. arXiv: 1706.03762.
Tongzhou Wang, Jun-Yan Zhu, Antonio Torralba, and Alexei A Efros (2018). “Dataset distillation.” arXiv:
1811.10959.
...
Yue Wu, Zhiqing Sun, Huizhuo Yuan, Kaixuan Ji, Yiming Yang, and Quanquan Gu (2024). “Self-play
preference optimization for language model alignment.” arXiv: 2405.00675.
Mengzhou Xia, Sadhika Malladi, Suchin Gururangan, Sanjeev Arora, and Danqi Chen (2024). “Less: Selecting
influential data for targeted instruction tuning.” arXiv: 2402.04333.
13
...
(2024). “Self-rewarding language models.” arXiv: 2401.10020.
Ruiqi Zhang, Licong Lin, Yu Bai, and Song Mei (2024). “Negative preference optimization: From catastrophic
...
Yulong Chen, et al. (2023). “Siren’s song in the AI ocean: a survey on hallucination in large language
models.” arXiv preprint arXiv:2309.01219.
Yao Zhao, Rishabh Joshi, Tianqi Liu, Misha Khalman, Mohammad Saleh, and Peter J Liu (2023). “Slic-hf:
Sequence likelihood calibration with human feedback.” arXiv: 2305.10425.
14
...
MORE RELATED WORKS
A.1
MORE ABOUT LEARNING DYNAMICS
Instead of emphasizing the model’s convergence status and guarantees, learning dynamics focus more on its
relative behavior during training. Since many LLMs exhibit “emergent ability” during training in terms of
the amount of training data or inference time (Schaeffer et al. 2024), like the “aha moment” mentioned in
D. Guo et al. (2025), we hence believe the analysis of learning dynamics can provide a novel perspective on
understanding the LLM system.
...
fθ(xo) keeps decreasing when it learns from xu. By studying the influence of different xu at different stages
during supervised learning, Ren et al. (2022) explain a “zigzag” pattern of the learning path, which sheds
...
why successive knowledge transferring can improve the model’s systematic generalization ability.
Besides explaining the model’s behavior, learning dynamics is also helpful for evaluating the quality or the
...
etc. Since the analysis throughout this paper relies on the “teacher forcing” mechanism and the relatively stable
...
(PPO, Schulman et al. 2017)1. We leave the study of the token-wise learning dynamics, which aligns better
with the “squeezing effect” in real settings, to future work.
...
use y+
j̸=i when answering question i. Holtzman et al. (2020) related the peakiness of the model’s distribution
to LLM’s “repeater phenomena”, which also indirectly supports our claims well: more DPO leads to a more
serious squeezing effect, hence the model’s prediction becomes peakier on most tokens, which makes the
...
Published as a conference paper at ICLR 2025
Furthermore, the “confidence decaying on y+
u ” attracts more attention in the community, because it is quite
...
careful when working on the “valley” region of the model’s distribution. Other works try to analyze this
problem by inspecting the token-level influence between responses. For example, Pal et al. (2024) assumes
y+
u and y−
u are identical expect one token. Under this assumption, the model’s confidence of y+
u after the
identical token is guaranteed to decrease. They propose a solution by significantly enhancing the learning rate
(roughly x50 larger when their λ = 50) of the positive part when detecting yu located in a low-confidence
...
A.3
BENIGN AND HARMFUL NEGATIVE GRADIENT
The “squeezing effect” can negatively impact our analysis when it is strongly imposed in a valley region
of the model. However, a well-regulated negative gradient is both beneficial and commonly observed in
many deep-learning systems. For example, it is common in many “machine unlearning” algorithms, e.g., in
...
is usually smaller than that used in SFT, which unintentionally mitigates the harmful squeezing effect. The
...
negative gradient (when its ˆAt is negative) on the valley region (when its πθ is small).
On the other hand, the effect that negative gradients make the model’s distribution peakier is independently
...
“PG-average” method is stronger than the standard PG method. Note that the “PG-average” method will
...
B
PROOF OF PROPOSITIONS AND RESIDUAL TERM FOR DIFFERENT LOSSES
B.1
...
Published as a conference paper at ICLR 2025
Proof. 2 Suppose we want to observe the model’s prediction on an “observing example” xo. Starting from
Equation (2), we first approximate log πt+1(y | xo) using first-order Taylor expansion (we use πt to represent
πθt interchangeably for notation conciseness):
log πt+1(y | xo) = log πt(y | xo) + ⟨∇log πt(y | xo), θt+1 −θt⟩+ O(∥θt+1 −θt∥2).
Then, assuming the model updates its parameters using SGD calculated by an “updating example” (xu, yu),
we can rearrange the terms in the above equation to get the following expression:
∆log πt(y | xo) = log πt+1(y | xo)
|
{z
}
V ×1
...

.
In the decomposition, using {π1, . . . , πV } to represent the model’s prediction on different dimensions, we
can write our At as:
At(xo) = I −1(πt)⊤=


...
the network’s notion of “similarity” evolves. For appropriately initialized very wide networks trained with
...
Kt(xo, xu) is unchanged (usually used in theoretical analysis) might be too strong in the LLM’s finetuning.
Hence as stated in the main context, our qualitative analysis only assumes that “during the training, the
relative influence of learning xu on all other different xo is relatively stable”. We will validate this assumption
using experiments in Appendix C.
2Note that this proposition assumes L = 1. For L > 1 case, we will have multiple task heads which leads to L
different Equation (3). The V × L matrix ∆log πt can then be achieved by stacking them.
17

Published as a conference paper at ICLR 2025
B.2
RESIDUAL TERM FOR DIFFERENT LLM FINETUNING ALGORITHMS
...
different l is independently generated as long as the shared network is fixed, the yl for the LLM’s output
depends on y<l, which is usually sampled from the model’s prediction iteratively. However, in most of
the finetuning cases where the supervisory signal yu is given, the model will apply the so-called “teacher
forcing” mechanism when calculating the predicting probabilities. In other words, when generating the
output of each yl, the y<l is given rather than sampled on-policy. This mechanism makes it possible for us
to define χ = [x; y] and hence merge the auto-regressive nature of the sequence prediction into the shared
Kt(χo, χu). After this step, the decomposition of LLM’s finetuning learning dynamics then becomes similar
to a multi-label classification task.
B.2.1
INSTRUCTION FINETUNING USING AUTO-REGRESSION LOSS (SFT)
Here we derive the residual term, i.e., Gt for different algorithms in LLM’s finetuning. We first rewrite
Equation (5) here:
[∆log πt(y | χo)
|
{z
...
where m ∈{1, . . . , M}, l ∈{1, . . . , L}, and Gt(χu) = ∇zL(χu)|zt is a V × L matrix. As the auto-
regression nature of the SFT loss is already encoded in the causal mask used in hθ, as demonstrated in
...
To calculate the equation above, we first recall the NLL loss of the l-th token is [LSFT]l ≜L = −log π(yl =
y+
l ) = −e⊤
y+
l log π, where π = Softmax(z). Then, ∇zL
|{z}
1×V
= ∇πL
|{z}
...
|{z}
V ×V
. For each dimension of ∇zLl, we
have ∂L
πi = 0 if πi ̸= y+
l and ∂L
πi = −1
πi if πi = y+
l . By writing it in vector form, we have ∇zL =
−(ey+
l ⊘π)⊤∇zπ. For ∇zπ, we have:
∇zπ =


π1(1 −π1)
...
Published as a conference paper at ICLR 2025
Combining this matrix and the 1 × V vector (ey+
l ⊘π)⊤, where the only non-zero term is
1
πk at the k = y+
l
position. So, left multiplying by this vector is actually first selecting the k-th row of ∇zπ, and then multiplying
1
πk to it. In summary, we have:
∇zL = −1
πk
[−πkπ1, −πkπ2, . . . , −πk(1 −πk), . . . , −πkπV ]⊤= [π1, π2, . . . , πk −1, . . . , πV ]⊤= π −ek
By stacking the terms with different l ∈[L], we can get
Gt
SFT(χu) = ∇zLSFT(χu)|zt = πθt(y | χu) −y+
u
(11)
B.2.2
DIFFERENT PREFERENCE FINETUNING ALGORITHMS
...
tion, because the loss term now depends on both πθt(y+
u | χ+
u ) and πθt(y−
u | χ−
u ), which involves
two different z terms. Specifically, we define πθt(y+
u | χ+
u ) = Softmax_column(z+) and πθt(y−
u |
χ−
u ) = Softmax_column(z−), where z+ = hθ(χ+
u ) and z−= hθ(χ−
u ) respectively (χ+
u = [xu; y+
u ]
...
Published as a conference paper at ICLR 2025
where [·; ·] are concatenation of two vectors or matrices, Gt
DPO+(χ+
u ) ≜∇z+LDPO, and Gt
DPO-(χ−
u ) ≜
∇z−LDPO. To calculate the residual terms, we decompose the loss into:
LDPO(xu, y+
u , y−
u | θ) = −log(a)
a ≜σ(b)
...
,
(14)
where c is not a function of θ. Using the chain rule, the l-th column of the residual term Gt
DPO+ can be
...
usually correlated with the confidence gap between the model’s current confidence on y+
u and y−
u , i.e.,
20
...
πθt(y−
u |χ−
u ). Generally, larger this value leads to a bigger a, making the norm of Gt smaller.
In other words, we see a “regularizing” effect in this term, where the model should not make Gap(πθt)
too large. The SLiC loss can be considered as a combination of SFT adaptation and preference adaptation.
Similarly, we can also see a hard version of the regularization effect mentioned above. If Gap(πθt) > δ, the
indicator function will become zero, and the model stops pushing π(y+
u ) and π(y−
...
KTO, IPO, SPIN, etc.) is that there is no negative sign in front of πθt(y+
u | χ+
u ) or πθt(y−
u | χ−
u ). From its
...
performs better.
C
THE “RELATIVE STABLE” ENTK ASSUMPTION
We use this appendix to verify the core assumption of our analysis – during the training, the relative influence
of learning xu on all other different xo is relatively stable – on both MNIST and LLM finetuning settings. To
make the notation concise, we use Kt
uo to reprsent Kt(xo, xu), Kt(χo, χu) and other related variants.
0
...
1
0 0
1 1
9 9 9
• Step1: Create probing dataset (#200)
20 examples for each number
• Step2: Calculate the NTK tensor each epoch
[  Epoch,    200, 200,      10, 10   ]
𝒦𝑡(𝑥𝑢, 𝑥𝑜)
(𝑜, 𝑢)
𝑡
• Step3: Calculate the NTK-norm tensor each epoch
[  Epoch,   200,   200,  1]
𝒦𝑡𝑥𝑢, 𝑥𝑜
𝐹
• Step4: Average the NTK-norm of different numbers
CoNTK ≜[  Epoch,   10,   10]
...
Align it with train/test loss,  𝒖,𝒐𝓚𝒖𝒐
𝒕
𝑭,
and  𝒖,𝒐𝓚𝒖𝒐
𝒕
−𝓚𝒖𝒐
𝒕−𝟏
𝑭together (observe different phases).
• Fact 1: although eNTK might change during training, the
relative relationship of CoNTK : , u, 𝑖on different 𝑖
is generally stable after several epochs (e.g., 50 epoch)
• Fact 2: the pairing effect of “4” and “9”, “5” and “8” is clear.
• Fact 3: change of eNTK in the late training (similar as finetuning)
is way smaller than the beginning phase.
Figure 6: Experimental design of verifying the relative stability of ∥Kt
uo∥F for fixed xu on different xo.
C.1
RELATIVE STABLE ENTK ASSUMPTION - MNIST EXPERIMENTS
...
uo∥F for fixed xu on different xo (labeled by the colorful
...
epochs (0 ∼30) are a bit messy, and the last several epochs (800 ∼1000) behave similarly to the
finetuning stage;
2. Although
the
norm
...
uo ∥F

) changes a lot after 30 epochs, the ranking between ∥Kt
uo∥F on different o
...
C.2
RELATIVE STABLE ENTK ASSUMPTION - LLM EXPERIMENTS
Directly calculating ∥Kt
uo∥F for the LLM experiment requires huge amount of computation, because for
each token in each example, we need to multiply a V × d matrix to a d × V one, where d is the number of
parameters of the LLM. However, since we only care about the relative relationship between ∥Kt
uo∥F on
22

Published as a conference paper at ICLR 2025
different χo, where χu is fixed, based on the basic decomposition in Proposition 1, we can get a lower-bound
as follows (ignoring superscript t for conciseness, ignoring the influence of O(η2)):
∆log π = −ηAoKuoGo
(23)
∥∆log π∥2
F = ∥−ηAoKuoGo∥2
...
Regarding the calculation of LBKuo, ∥∆log π∥2
F is easy to track because, in the main context, we already
showed log πt for different responses. ∥Go∥2
F = ∥π −y+
u ∥2
F , where y+
u is defined as a stacking of L one-hot
vectors. The ∥Ao∥2
F is a bit complex. Recall the definition that Ao = I −1π⊤, we can have:
∥Ao∥2
F = Trace
 A⊤
o Ao
...
 I⊤I −π1⊤−1π⊤+ π1⊤1π⊤
(29)
= Trace(I⊤I) −2Trace(1⊤π) + V Trace(π⊤π)
(30)
= V −2 + V ∥π∥2
2,
(31)
which is also trackable in our setting. Note that intuitively, the value of ∥π∥2
2 is inversely correlated to the
Shannon entropy of the distribution π: ∥π∥2
2 = 1 if π is one-hot; ∥π∥2
2 =
1
√
V if π is uniform. Hence we
can also interoperate ∥Ao∥2
F as the peakiness of π(y | χo). In the following experiment, we track the value
...
4. An interesting “similarity pattern” occurs: by observing SignDeltauo, we see SFT using y+
gpts or
y−
...
5.
By observing the last row, where the model is trained using DPO, it is clear that the push-down
...
SFT using (xu, y +
u )
Tracking lower bound of eNTK: LBKt
uo
y +
u
yinit
...
Auo 2
F
Average Peakiness Change during SFT
y +
u
yinit
y +
...
40
20
Average of log (y * ) and log (yc) during SFT
argmax
except_argmax
linear fit of log (pc)
0
10
20
30
...
40
20
Average of log (y * ) and log (yc) during DPO
argmax
except_argmax
linear fit of log (pc)
Figure 9: Other metrics related to LLM’s learning dynamics. The first panel demonstrates how ∥At
o∥2
F
changes during SFT (higher means peakier π). It is clear that the peakiness of y+
u , i.e., the supervisory signal,
increases fastest. The last two panels demonstrate the average log π(y∗) and its complementary (denoted by
log π(y∗)C, which measures how many probability masses are left for other possible tokens). The second
one is for SFT and the third one is for DPO. It is clear that log π(y∗) and log π(y∗)C changes faster in the
DPO case, which matches our observations in the fourth panel of Figure 3 well. The linear fit extrapolates the
log π(p∗)C values because we suffer an underflow issue when estimating this term. We will fix them in the
...
1. Chosen response
1.1 Self-rephrase
1.2 GPT-rephrase, semantics keeping
1.3 GPT-rephrase, format keeping
2. Rejected response
2.1 Self-rephrase
2.2 GPT-rephrase, semantics keeping
2.3 GPT-rephrase, format keeping
3. Irrelavent from train
4. Irrelavent from test
5. Random sentence
6. Randomly permuted 𝐲u+
...
D.1
THE SELECTION OF RESPONSE TYPES FOR THE PROBING DATASET
Besides the sequential nature of the loss function, another conundrum in analyzing LLM learning dynamics is
the huge response space Y: the number of possible y ∈Y is V L, but the vast majority of possible sequences
...
can be understood as “how suitable this y is as a response to xu, compared to y+
u .” Then, starting from
...
Published as a conference paper at ICLR 2025
• YIF: reasonable responses following the instruction xu:
0. yπ0, the initial response generated by feeding xu to LLM before finetuning;
1. y+
u , the chosen (i.e., the preferred) response to xu.
1.1 y+
selfr, rephrase y+
u using yπ0, algorithm from Z. Yang et al. 2024;
1.2 y+
gpts, rephrase y+
...
gptf, rephrase y−
u using ChatGPT, keep the format while changing the semantics;
• Ynon-IF: irrelevant responses to xu that are still recognizably human language (in these datasets,
roughly “internet-standard” English):
3. y+
j̸=u, the chosen response for a different question xj̸=u selected from the training set.
4. y+
test, the chosen response of a question xtest selected from the test set.
5. yhum, a “random” English sentence generated by ChatGPT with as many words as y+
u .
...
Compared with Dprobtest that we used in the main context, all the prompts and responses in Dprobtest are never
exposed to the model during finetuning. By comparing the learning curves of these two probing datasets, we
can figure out the difference between the model’s prediction of those directly influenced responses (y appears
during training) and the indirectly influenced ones (y that the model never sees during training). Finally,
we believe the level of the “on-policy” property (which is very important for the preference finetuning, as
...
Below is an instruction that describes a task along with a reference answer. Using the reference answer as a guide, write your own
response.
### Instruction: {𝐱}
### Reference Answers: {𝐲+}
### Response:
•
Prompt for self-rephrase: 𝐲𝐬𝐞𝐥𝐟𝐫
+
•
Prompt for GPT-rephrase semantics keeping 𝐲𝐠𝐩𝐭𝐬
+
Given the reference sentence, please generate an output sentence. Please use different words as much as possible while keeping the
meaning of the reference sentence unchanged. Please only return the output sentence.
### Reference sentence: {𝐲+}
### Response:
•
Prompt for GPT-rephrase format keeping 𝐲𝐠𝐩𝐭𝐟
+
Given the reference sentence, please generate an output sentence. Please change the meaning of the reference sentence as much
as possible while keeping the format of it. Please only return the output sentence.
### Reference sentence: {𝐲+}
### Response:
...
similarly. The self-rephrase template comes from Z. Yang et al. (2024).
26

Published as a conference paper at ICLR 2025
Human:    What will remove cigarette odor from a room?
Assistant: I believe there are products that will work at removing odors.  Can you
tell me what’s going on?
Human:    What do you mean?
Assistant:
Prompt: 𝐱
0. Initial response
I’m not a chemist, but I’m guessing that most of the cigarette odor comes from the
burning tobacco.  I know we can buy products that reduce the smell of smoke, and
that is what most manufacturers claim.  Would that also work to remove cigarette
odor in your apartment?
1. Chosen response
You mentioned something about a cigarette smell, can you tell me what happened?
1.1 Self-rephrase
I believe there are products that will work at removing cigarette odor.
1.2 GPT-rephrase, semantics keeping
You brought up the issue of a cigarette odor, could you provide me with
more details?
1.3 GPT-rephrase, format keeping
Can you provide details on the odor you noticed earlier?
2. Rejected response
If you tell me about your situation, I can try to give you more help.
2.1 Self-rephrase
2.2 GPT-rephrase, semantics keeping
Should you share details of your circumstances with me, I will endeavor to provide further assistance.
2.3 GPT-rephrase, format keeping
If you share your circumstances with me, I'll do my best to provide you with additional assistance.
3. Irrelavent from train set
Let’s take a look at the next month’s release schedule. Are there any games you’re particularly
excited for?
4. Irrelavent from test set
You’ve searched through quite a few results and haven’t come across a recipe you like yet.
5. Random sentence
The purple cat danced under the starry night sky with joyful abandon.
6. Random permuted 𝐲𝐮+
me about mentioned can smell, tell happened? You cigarette something you a what
7. Pure random tokens
you a through few You’ve recipe yet. and across quite a searched come haven’t results like
I’m not sure what you mean. I’m not sure what you’re asking.
...
D.2
MORE RESULTS ON DIFFERENT SETTINGS: SFT CASE
...
into the same figure. We can treat the red curve, i.e., the one of y generated by π0(x), as a baseline, whose
decaying suggests the policy model is deviating from the initial point. The first observation is that after
several updates, y+
u is the only one that keeps increasing fast, which means the “pull up” pressure generated
by [xu; y+
u ] do not have that strong influence on these rephrases compared to [xu; y+
j̸=u], even though these y
are good rephrases of y+
u (recall the curve y+
j̸=n always increase in Figure 14). Furthermore, by carefully
comparing the decreasing speed of yπ0 and other curves, we find those rephrases decays slower than yπ0 in
...
u , their rephrases are “pulled up” more than the rephrases of y−
u . Such
...
u is now identical
to that of yπ0 while the decaying speed of rephrases for y−
...
Compare Dprob and Dprobtest. To isolate the influence of the “pull up” pressure introduced by the training
...
gpts) >
trend_diff(y+
gptf), which aligns well with our hypothesis about how strong the “pull up” pressure influence
different responses.
0
2
4
...
136
134
Whether y is a dialog response
y +
test
0
5
...
20
25
Inflection time of y +
test
Updates before inflection
140
130
120
110
...
395.0
392.5
390.0Whether y is a dialog response
y +
test
0
5
...
3.) y+
j̸=u increases with a smaller rate than y+
u , although the [xu; y+
j̸=u] never occurs during training; 4.) both
y+
...
u than yhum (i.e., larger ∥Kt∥F ). Hence in general, the “pull-up” pressure
on y+
...
gpts and y+
gptf are smaller than yπ0, which means the pull-up pressure
exists; 2.) For the second row, the decaying speed of y−
gpts and y−
gptf are similar to that of yπ0, because the
...
gpts, y+
gptf and y+
j̸=u shows a clear trend. (Antropic-HH, SFT)
30

Published as a conference paper at ICLR 2025
D.3
MORE RESULTS ON DIFFERENT SETTINGS: OFF-POLICY DPO CASE
...
1.) in the first column, the margin πθt(y+
u ) −πθt(y−
u ) keeps increasing. The πθt(y+
u ) first increase and
then decrease, always with a smaller decay speed than that of πθt(y−
u );
2.) in the second column, πθt(y+
u ) decreases slower than the other rephrases, verifying the “pull up”
pressure and the influence on other responses via Kt;
3.) in the third column, πθt(y−
u ) decreases faster than the other rephrases, verifying the “push down”
pressure and the influence on other y;
4.) in the fourth column, the rephrases of y+
u decay slower than those of y−
u , supporting the claims that
the rephrases near the chosen responses are influenced by the “pull up” pressure while the rephrases
of the rejected ones are influenced by the “push down” pressure.
...
practice in many existing works. Hence in this part, we plot the curves of different πθt(y) in both two stages
...
controlled to be the same (i.e., 5 × 10−7, the default value in (Tajwar et al. 2024)). All the curves are aligned
...
hold in these experiments. We then compare the model’s behavior in these two phases respectively. In the
last two rows of Figure 17, where the epoch for SFT is non-zero, it is clear that the decaying speed of most
observing πθt(y) is much larger in DPO than those in SFT. The main reason for this is the existence of a
big negative gradient introduced in DPO. This gradient, especially conducted on a “valley” region of the
model’s prediction, will “push down” the whole curve significantly, except the one with the highest confidence
before updating. This non-trivial trend is named “squeezing effect”, which is elaborated on in Appendix E.
Furthermore, a more peaky πθ0(y) and a smaller πθ0(y−
u ) will lead to a stronger “squeezing effect”, which
can be verified by comparing the curves of the last two panels: longer SFT makes the model’s prediction
peakier when DPO is conducted, which leads to a larger decay on all πθt(y) during DPO.
31
...
serious the squeezing effect is (confidence decays faster). (Antropic-HH, SFT →DPO)
0
1
2
3
...
y+
u decays slower than that of y−
u ; 2.) Confidence of y+
...
u ; 3.) Confidence of y−
u decays faster than those of y−
gpts and
y−
gptf, because the push-down pressure is directly imposed on y−
...
Published as a conference paper at ICLR 2025
E
THE SQUEEZING EFFECT INTRODUCED BY BIG NEGATIVE GRADIENT
...
panel in Figure 2. These two opposite pressures ensure the margin reward πθ(y+
u | χ+
u ) −πθ(y−
u | χ−
u )
...
u ) and πθ(y−
u | χ−
u ) separately (actually we should, because their χu are usually
different), a very interesting phenomenon occurs. See the first column of Figure 18, we find although DPO
also contains a strong positive adaptation vector, the curve of πθ(y+
u | χ+
u ) all goes down after several updates,
which is very different from πθ(y+
u | χ+
...
observed πθ(y) keeps decreasing during training. So, it is natural to ask:
Where has the probability mass gone?
E.1
THE SQUEEZING EFFECT AND WHY IT EXISTS
...
considers the negative gradient, i.e., the one imposed by y−
u in LLM’s finetuning. As also pointed by Razin
et al. (2025), the pull up pressure imposed by y+
u will cancel the influence imposed by y−
u when their χu
are identical. However, when χ+
u and χ−
...
to a length-d feature vector via a deep neural network ϕ. In other words, we have ϕ(x) ∈Rd×1. The model
uses a linear read-out layer w ∈Rd×V to convert the feature vector to logits z = w⊤ϕ(x) and then generate
...
truth label y. To simplify our analysis, we assume a fixed ϕ and only update the parameters of the read-out
layer w using stochastic gradient descent:
wt+1 = wt −η∇wL = wt −ηϕ(x)(pt −ey)⊤,
(33)
where η is the learning rate which can be negative if we consider a negative gradient during training. With
Equation (32) and (33), we can write down each dimension of pt and pt+1 after some calculations. To
quantitatively analyze how the model’s confidence in each class changes, we define a ratio αi ≜pt+1
i
pt
i
and use
...
Published as a conference paper at ICLR 2025
Note that the values of βj also depends on whether i equals y, hence for Case 1 (i = y) and Case 2 (i ̸= y),
we have (η′ ≜η∥ϕ(x)∥2
...
j=1 e
zt
j , we can derive αi and βj as the left part of Equation (35). Similarly,
when i ̸= y, we have:
pt+1
i̸=y =
ezt+1
...
We can now better understand how each pi changes after this update. Specifically, if αi > 1, the corresponding
pi increases, and vice versa. To determine the value of αi, we can treat any βj > 1 as contributing to the

## Conclusion

j and |βj −1| controls
how strong the contribution is. With the preparations above, we derive the following observations on how the
confidence evolves when a gradient ascent (i.e., η < 0) is imposed on class y.
34

Published as a conference paper at ICLR 2025
Claim 1: The value of py is guaranteed to decrease, i.e., αy < 1.
We start from the value of β in Case
1 as illustrated in Equation (35). It is clear that for any j ̸= y, we have βj > 1, because 1 + pt
j −pt
i > 0.
Combining with βy = 1, it is straightforward to have Claim 1.
Claim 2: The value of pi∗where i∗= argmaxi∈[V ]\{y} pt
i is guaranteed to increase, i.e., αi∗> 1.
We
now use the value of β in Case 2, since i∗cannot equal y by definition. When j ̸= y, we have pt
j −pt
i∗≤0
for all possible j, because pt
i∗is the largest among all pt
i̸=y of pt. Hence all βj̸=y must be smaller than one.
Combining with the fact that βy < 1 (because pt
y −pt
i∗−1 must be negative), we can prove that αi∗> 1.
...
to be “squeezed” into the dimension with the highest pt
i∗(if pt
...
into other pi (i.e., other pi increases)? Or will pi∗absorb the mass not only from py but also from other
...
distributed to all other i ̸= y, i.e., all pt+1
i̸=y increase the same value.
With the uniform pt assumption,
Equation (34) can be simplified to αi =
V
PV
j=1 βj . Note that the first two claims hold for any distribution
pt, hence we only check the values of αi̸=y here to verify the “uniformly distributed mass” hypothesis.
Substituting the values of βj to this new α leads to αi =
V
V −1+eη′ for all i ̸= y. Since η′ < 0 and eη′ < 1,
we must have αi̸=y > 1. Combined with the fact that all pt
i are the same, this claim can be proved.
Claim 3B: When pt is slightly non-uniform, pi with smaller pt
i tend to decrease, and vice versa.
This
claim is a general trend and might not have any guarantees. However, analyzing such a scenario helps us to
understand the influence of pt better. Assume we are observing αi′ where i′ is not y nor i∗. We consider two
...
βj∈B > 1.
(40)
Note that we misuse the ≪notation to highlight the fact that βj=y would be much smaller than βj∈S, because
there is a negative one term in the exponential. With the above expression, we can imagine that if pt
i′ is
relatively small, the size of B would be large, which means there will be more βj > 1 contributing to the
conclusion that αi′ < 1. If the influence of βj∈B is strong enough to override the influence of other β
(especially βj=y which is way smaller than other β), αi′ would be smaller than one and hence pi′ decreases.
On the contrary, for those i′ with relatively large pt
i′, the β < 1 terms becomes dominant and hence lead to
αi′ > 1, i.e., pi′ increases.
In the analysis above, we assume pt is only slightly non-uniform (i.e., not so peaky), which means the
values of different ezt
j are relatively comparable. However, in practical machine learning systems like LLM’s
finetuning, the distribution pt would be very non-uniform, which means most of the probability mass is
obtained by a few dimensions. That is because the LLM’s vocabulary size is usually very large and the
...
Published as a conference paper at ICLR 2025
Claim 3C: When pt is very peaky, which means most of the probability mass is obtained by i∗, then
all other pi will decrease. In other words, the probability mass of all other pi is squeezed to pi∗.
We
...
i∗that dominates α. In other words, αi ≈
ezt
i∗
βi∗·ezt
i∗=
1
βi∗. Then for any i′ we want to
observe, the αi′ ≈
1
βi∗< 1. In other words, the model’s predictions on all dimensions other than the one
with the highest confidence in pt will decrease.
Last, we analyze the influence of py to explain why “imposing a large negative gradient on the valley region”
makes the squeezing effect more serious.
Claim 4: Smaller pt
y makes those non-max pi easier to decay, i.e., a stronger squeezing effect.
This is
...
illustrated in Equation (40), where the value of β is decomposed into three subgroups. Recall the definition of
αi, we know all βj < 1 contribute to the hypothesis that pi increases after this update, where the strength
of this contribution is controlled by ezt
j. Since a pt
y small means a small ezt
j, the influence of βj=y ≪1 is
significantly weakened under this scenario. In other words, αi < 1 is more likely to occur for all possible i,
which means the squeezing effect (all pj̸=y decreases) becomes more serious.
Claim 5: The learning rate with a larger absolute value |η| and a larger feature norm ∥ϕ(x)∥2
2 will
amplify all the trends, maybe more serious than our expectation.
Throughout our analysis, the equivalent
learning rate η′ < 0 is a shared scalar in all βj. Hence larger |η′| can amplify all the trends aforementioned.
Furthermore, recall the shape of an exponential function ex, where a small change of x (especially when x > 1)
will make ex changes a lot. Then the terms βj̸=y = e−η′(1+pt
j−pt
i) in Case 1 and βj=y = e−η′(pt
j−pt
i−1) in
Case 2 will play a stronger role if we use a larger learning rate |η| or the norm of features is larger.
E.2
VERIFY THE SQUEEZING EFFECT USING A SIMPLE EXPERIMENT
Figure 19: Illustration of how big positive and negative gradients influence the model’s prediction.
Let us analyze a simple example to get an intuition. We set V = 50, d = 5, |η| = 0.5, and a randomly
generated ϕ(x). In the first row of Figure 20, we consider the model updates its parameters using standard
...
demonstrate the model’s predicted distribution before and after this update. As we expected, the positive
vector on the 21st class “pull up” p0(y = 21) and “push down” all other p1(y) at the same time. This trend is
quite consistent under different settings (i.e., different choices of V, d, x, η, w0, etc.), which can be depicted
...
y +
u
Probability change in log scale
0
10
20
30
...
3
yu
Figure 20: Experimental verification of the “squeezing effect” illustrated in Figure 19 using a simple multi-
class logistic regression task.
We then set η = −0.5 to simulate the negative gradient in DPO and consider three different settings. First,
we assume the model’s prediction on x is relatively flat, as demonstrated in the second row of Figure 20,
where the predicting probability of every class is around 0.02. The negative gradient is imposed on y = 42,
a randomly selected number. We see the negative adaptation vector “push down” p1(y = 42) heavily and
re-assign those decreased probability mass evenly to all other classes, as illustrated in the second panel in
Figure 19.
Although the behavior described above follows our intuitions well, a flat p0 is not common in LLM’s
finetuning. Because finetuning usually starts from a pre-trained w, where the model’s prediction would likely
...
use η = −0.5 to “push down” the model’s prediction on this class. As demonstrated by the blue curve,
37

Published as a conference paper at ICLR 2025
p1(y = 11) decreases a lot as we expected. However, different from the flat p0 case, where the model evenly
assigns the reduced probability mass to all other y, the model in this example “squeezes” the mass to those
confident predictions, i.e., classes 6, 9, and 10, leaving the confidence of other classes almost unchanged.
Such a trend is consistent when the negative gradient is imposed on the “peaky” region of a non-uniform
distribution, as illustrated in the third panel in Figure 19.
The previous setting simulates the on-policy DPO well, where the rejected examples y−
u are sampled from
the high confidence region of the model’s predictions. Then, what will happen if we conduct off-policy
DPO and impose a big negative gradient on those classes that already have very low confidence? See the
fourth row of Figure 20, where we use the same w0 and η as in the previous case. The only difference is
that we change the label of x to 42, where p0(y = 42) is very small (roughly 10−5) before training. The
behavior in this setting is quite interesting: we first observe a big increase on p1(y = 11), which means the
model “squeezes” the probability mass to the most confident one in p0, similar to the previous setting. More
interesting, the predictions on all other y are heavily “pushed down”, even including classes 6, 9, and 10,
...
depends on ∥ϕ(x)∥, becomes larger). Since the vocabulary size of a common LLM is usually more than 50k,
the squeezing effect in real systems would be non-negligible even if the learning rate is small. Such a trend
is also quite consistent as long as we impose a big negative gradient on the “valley” region of the model’s
prediction, as illustrated in the last panel in Figure 19. Now we can answer the question of why all observing
πθt(y) decreases and where the probability mass has gone:
...
the prior knowledge contained in θ0, which could be a drawback for off-policy DPO.
The hypothesis above is not only supported by this simple logistic regression problem but also by many
consistent trends in LLM’s finetuning experiments. First, by comparing the average decaying speed of the
πθt(y) when the model SFT different epochs before DPO (in Figure 17), we notice that longer SFT leads to a
more peaky πθ0(y) and hence leads to a faster decaying speed of all non-argmax responses. That is because
the longer SFT stage will eventually push down πθ0(y−
...
it “argmax confidence”, i.e., P
l πθt(argmaxyl∈Ylyl | x, y1:l−1). As illustrated in the last panel in Figure 4,
the argmax confidence keeps increasing while all other πθt(y) decreases: the missing probability mass is
found! Last, in the dataset-extension method we proposed in Section 4.3 and Appendix F, we train the model
using both [x, y+
u ] and [x, y−
u ] during SFT to also “pull up” the y−
...
flow has a lower “argmax confidence” during DPO. That is because we pulled up πθ0(y−
...
model’s prediction. Such a change in turn weakens the squeezing effect, as illustrated in Figure 5.
F
A SIMPLE METHOD TO IMPROVE ALIGNMENT
F.1
PINPOINTING THE DRAWBACK OF OFF-POLICY DPO
Based on our observations and analysis above, we speculate that “imposing big negative gradients on the
valley region” is one of the bottlenecks of off-policy RL-free methods. Starting from this hypothesis, we
...
Published as a conference paper at ICLR 2025
that these methods improve the model’s performance not only by mitigating the squeezing effect. Hence
to figure out to what extent the squeezing effect can harm the model’s performance, we propose a simple
...
F.2
A SIMPLE METHOD INSPIRED BY LEARNING DYNAMICS
...
can pull up the region of supervised ˆy and we don’t want the model to impose big negative gradients on a
valley region, we can just pull up those y−
...
u and y−
u well. In other words, under DPO’s
loss, there is no need to worry about the model overfitting those y−
u during SFT.
F.3
...
these two methods. Model B2−2/4/6 and E2−2/4/6 are the models finetuned using DPO for 2/4/6 epochs. All
...
analysis quite well. See the first two panels that compare πθt(y+
u ) and πθt(y−
u ) respectively. It is clear that
these two methods have an almost identical curve on πθt(y+
u ) in the SFT stage but behave quite differently
on πθt(y−
u ): because we directly train the model using (x, y−
...
That is the benefit introduced by “pulling up” the πθ0(y−
...
the SFT stage, the observed “argmax-probability” of the proposed method is higher than the baseline, because
39
...
u during SFT; 2.) The extend method considerably increases y−
u during
...
we impose twice “pull up” pressure, i.e., those for (x, y−
...
exactly imposed on those y−
u (in the second panel of Figure 5, πθ0(y−
u ) is already very high). Furthermore,
at the end of DPO, we see the “argmax-probability” of the proposed method is significantly lower than the
...
squeezing effect, indeed does alignment better, we conduct pair-wise comparisons of these models’ responses
...
a. B2−4 v.s. B1, win rate is 0.6727 and 0.6411;
b. E2−4 v.s. E1, win rate is 0.6898 and 0.7321;
3. Compare the proposed method and baseline after DPO for different epochs:
a. E2−2 v.s. B2−2, win rate is 0.6518 and 0.5151;
b. E2−4 v.s. B2−4, win rate is 0.6928 and 0.6045;
c. E2−6 v.s. B2−6, win rate is 0.6667 and 0.5432;
4. Compare the best E2−4 with other 2 checkpoints:
40

Published as a conference paper at ICLR 2025
a. E2−4 v.s. E2−2, win rate is 0.6853 and 0.5517;
b. E2−4 v.s. E2−6, win rate is 0.6324 and 0.5316;
...
comparisons in this group are consistent: E2−4 brings the largest improvement, which is potentially the best
model. This fact is verified in the fourth group comparison, where we evaluate E2−4 against E2−2 and E2−6.
The results demonstrate that both a too-long or too-short finetuning stage using DPO is not the best choice.
Figure 23: Prompt used for evaluating model’s response (from (Rafailov et al. 2023)), an example feedback
