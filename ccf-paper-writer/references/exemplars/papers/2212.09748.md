...
tive models remain holdouts to the trend, thoughwhile
...
recent trend of architecture uniﬁcatione.g., by inheriting
...
More speciﬁcally, we study the scaling behavior of trans-
...
trained within a VAEs latent space, we can successfully
...
and Dhariwals seminal work improving diffusion mod-
els [9, 36] is most related to usthere, they analyzed the
...
Before introducing our architec-
...
N(xt; √¯tx0, (1 −¯t)I), where constants ¯t are hyperpa-
...
t DKL(q(xt−1|xt, x0)||pθ(xt−1|xt)), excluding an ad-
ditional term irrelevant for training. Since both qand pθ
are Gaussian, DKL can be evaluated with the mean and co-
variance of the two distributions. By reparameterizing µθ as
...
and Dhariwals approach [36]: train ϵθ with Lsimple, and
...
wise scale and shift parameters and , we regress
...
tion to regressing and , we also regress dimension-
...
d and attention heads [10, 63]. Speciﬁcally, we use four
...
early decode each token into a pp2C tensor, where C is
...
tmax = 1000 linear variance schedule ranging from 110−4
to 2 10−2, ADMs parameterization of the covariance Σθ
...
using ADMs TensorFlow evaluation suite [9]. FID num-
bers reported in this section do not use classiﬁer-free guid-
...
model quality.
Initialization is also importantadaLN-
...
our DiT models after 400K training steps using the same input latent noise and class label. Increasing the Gﬂops in the modeleither by
increasing transformer depth/width or increasing the number of input tokensyields signiﬁcant improvements in visual ﬁdelity.
...
We plot the Gﬂops of each of our DiT models and each models
...
formers total parameters are effectively unchanged (actu-
...
XL/2
Figure 9.
Larger DiT models use large compute more efﬁ-
...
Class-Conditional ImageNet 256256
Model
FID
sFID
Precision
...
Class-Conditional ImageNet 512512
Model
FID
sFID
Precision
...
ImageNet 512512. Note that prior work [9] measures Precision
...
XL/2 uses 5 less compute15.2 Tﬂopsto sample each
...
[22] Aapo Hyv¨arinen and Peter Dayan.
Estimation of non-
...
computer vision and pattern recognition, pages 11251134,
...
learning, pages 40554064. PMLR, 2018. 2
...
puting and computer-assisted intervention, pages 234241.
Springer, 2015. 2, 3
...
models in Table 4, including both 256 256 and 512 512
...
formers hidden size and SiLU activations. Each adaLN
...
transformers hidden size. We use GELU nonlinearities (ap-
...
results (in terms of FID) when simply adjusting the scale
factor.
Speciﬁcally, three-channel guidance with a scale
of (1 + x) appears reasonably well-approximated by four-
channel guidance with a scale of (1 + 3
4x) (e.g., three-
channel guidance with a scale of 1.5 gives an FID-50K of
2.27, and four-channel guidance with a scale of 1.375 gives
...
computed without classiﬁer-free guidance. Parameter and ﬂop counts exclude the VAE model which contains 84M parameters across the
...
metricsFID, sFID, Inception Score, Precision and Recall.
We ﬁnd that our FID-driven analysis in the main paper gen-
eralizes to the other metricsacross every metric, scaled-up
...
periments. The VAE models (ft-MSE and ft-EMA) are ﬁne-
...
Class-Conditional ImageNet 256256, DiT-XL/2-G (cfg=1.5)
Decoder
FID
sFID
Precision
...
variants against transformer Gﬂops, ﬁnding strong correlations across metrics. All values were computed using the ft-MSE VAE decoder.
