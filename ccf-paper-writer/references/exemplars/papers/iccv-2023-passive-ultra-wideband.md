...
an extreme range of timescales simultaneouslyseconds to
picosecondsand doing so passively, without much light,
...
tic calculus to enable reconstruction of a pixels time-
...
in the timestamp data, and (3) ensure the algorithms noise
...
everyday motionsbut can also be played a billion times
...
fast cameras [24], and depth sensors [5, 6] have made it
...
Acquiring videos of ultrafast phenomena this wayfrom
...
But how do we image highly dynamic scenesboth slow
and ultrafastpassively, without any light sources under
our control, no synchronization, and not much light? Very
little is known about this problem because existing mod-
els for passive low-light imaging [913] break down at
...
a scene. The only requirements are that (1) the cameras
...
to seconds)and that involve many unknown sourcescan
...
and two unsynchronized picosecond lasers (40 MHz10 GHz). Top right: By reconstructing the time-varying ﬂux function of the laser
...
capture refocusing in timefrom transient to everyday
...
21] and time series analysis [2227], and similar methods
...
ing ﬂux from photon data [13, 3133]. These rely on a va-
...
exclusively on sync-relative timestamps [3438], where in-
...
ﬂux at one speciﬁc modulation frequency [3943]. These
...
ing system exerts no control over a scenes appearance: the
scenes light source(s) can be natural, artiﬁcial, or both, and
...
sents the pixels instantaneous ﬂux at time t 0. Our goal is
...
tum efﬁciency refers to the pixels probability of actually
...
in time to fall within a SPADs dead-time window with high
probability [32, 48, 49]. For simplicity, we focus on low-
ﬂux imaging in the main paper, where consecutive arrivals
are spaced much farther apart than the SPADs dead time.1
...
1For example, Tavg was nearly six times our SPADs dead time in the
...
Section 3s results, graciously provided by anonymous re-
...
scenes ﬂux [32, 33]. This implicitly assumes that ﬂux does
...
in a relatively small number of time binstypically a few
timestamps effective number of bits is reduced [52]. Our systems times-
...
papers scope (e.g., jitter can actually improve timing resolution [5355]).
...
photon detections occur at a rate (much) lower than the ﬂux functions frequencies. If a light sources frequency is not a multiple of fsync
...
thousandand yields a photon-count histogram [57] that
...
cially, all frequencies from DC up to the Nyquist limitand
...
photons never accumulate, and the contents0 or 1of
...
SPADs timing resolution, so acquisitions of a second or
...
pixelhowever few or far apart they may beand (2) the
...
quency spectrum and are identical toand indistinguish-
able fromlower-frequency measurements:
...
probability of false alarm . False alarms occur when we
keep f and E[|p f (T )|] = 0; we remove f if pE
f (T ) is lower
than CDF−1
χ2 (1−), derived from Corollary 2. Speciﬁcally,
we detect frequencies for which
|p f (T )|2 CDF−1
χ2 (1 −)N(t)
(7)
Note that (1) for a ﬁxed , the probability of detecting a
...
camerasSPADs or otherwisewere thought capable of
acquiring directly [4143, 45], i.e., without resorting to ho-
...
didate frequenciese.g., a 1 Hz-resolution scan of DC-to-
20 GHzby rejecting spurious frequencies and reducing
storage requirements. The frequency detection step uses
the CFAR detector of Section 3, where we set so that
...
tor (60 Hz10 MHz), and (3) two unsynchronized picosec-
ond lasers (40 MHz10 GHz). Remarkably, the ﬂux func-
...
functions themselveswe are demonstrating a new capa-
bility of reconstructing the appearance of the scene as it ap-
peared during each laser pulsethis is distinct from the use
...
propagating through the bottle (Figure 5, rows 34). For
...
beams raster scan, collecting light that bounced twice (i.e.,
diffuserdiffuse boxSPAD). This conﬁguration is anal-
...
ure 5, rows 56) even though only 3000 photons were col-
...
speed video with a 3232 SPAD array. They assume ﬂux
...
each pixela dead time too long to ignore even at relatively
low ﬂux levelswe probe using the generalized algorithm
of Section C.1. This yields a periodic ﬂux function truer
to the rotating fans motion, whereas periodicity and high-
...
multiple light sources (right). Rows 34: Flux function images at two timescales (left, bottom right) and for different points in the scene
...
bottle (B) propagating to the cap (C), and reﬂecting back (D). Rows 56: Passive NLOS acquisition using a raster-scanning laser projector
with ground truth and reconstructed frames. Rows 78: We use SPAD array data from Seets et al. [13] to reconstruct per-pixel ﬂux (left).
...
Sens. Actuators A: Phys., vol. 140, no. 1, pp. 103112,
...
Proc. SPIE, vol. 3100, pp. 245253, 1997.
...
[12] M. OToole, F. Heide, D. B. Lindell, K. Zang,
...
Recognition (CVPR), pp. 15391547, 2017.
...
Computer Vision (WACV), pp. 19441953, 2021.
...
spectra, Biometrika, vol. 37, no. 1/2, pp. 116, 1950.
...
series, J. R. Stat. Soc., vol. 108, no. 1/2, pp. 93141,
...
[27] J. T. VanderPlas, Understanding the LombScargle
...
sion and Pattern Recognition (CVPR), pp. 85858595,
[34] D. B. Lindell, M. OToole, and G. Wetzstein, To-
wards transient imaging at interactive rates with
single-photon detectors, in Proc. IEEE Int. Conf. on
Computational Photography (ICCP), pp. 18, 2018.
...
eras, in Proc. IEEE/CVF Conf. on Computer Vision
and Pattern Recognition (CVPR), pp. 1785417864,
...
J. Sci. Comput., vol. 41, no. 5, pp. C479C504, 2019.
