...
| Accept a complete scientific-problem document, implement/audit a candidate MES and initial algorithm, repair failures, then freeze the first anchor. | `ccf-mes-validation` | Phase A document, environment/algorithm implementation, one-time L2, Ralph repair | Does not start later complexity stages or publication experiments. |
| Accept one versioned complexity-upgrade document, implement/audit the stage environment, then modify/audit/repair the algorithm while preserving the anchor. | `ccf-complexity-upgrade` | Phase B upgrade document, `stage_case`, anchor regression, algorithm repair | Never creates another MES, reruns L2, or changes the anchor. |
| Independently audit Phase-A or Phase-B environment code against the accepted phase document. | `ccf-env-code-auditor` | Phase-A L1/L2, Phase-B stage consistency, interface/leakage checks, anchor regression, native review | Does not design, implement, repair, or judge algorithm performance. |
| Independently audit initial or upgraded algorithm code, feasibility, correctness/convergence, and exact/oracle/bound evidence. | `ccf-algorithm-code-auditor` | equation-to-code trace, role gate, reference checks, anchor/stage acceptance, native review | Does not design, implement, or repair algorithms or environments. |
...
ccf-project-scaffolder
  ...
  -> ccf-mes-validation (Phase A: complete document -> candidate MES/environment -> initial algorithm -> frozen anchor)
  -> ccf-complexity-upgrade (Phase B: upgrade document -> stage environment/audit -> algorithm modification/repair; no new MES)
...
| `ccf-algorithm-designer` | `ccf-mes-validation`, `ccf-complexity-upgrade` | Initial algorithm design belongs to Phase A; stage modification and repair belong to Phase B. |
| `ccf-experiment-debugger` | `ccf-mes-validation`, `ccf-complexity-upgrade` | Each phase owns its append-only one-owner/one-delta Ralph rounds. |
...
| 在 accepted anchor 后按升级文档实现复杂场景、审查环境、修改并修复算法 | `ccf-complexity-upgrade` -> 两个 code auditor |
...
| 用 Ralph loop 在固定 MES 上完成一次复杂度升级 | `ccf-complexity-upgrade` |
| Phase-A 失败后执行一个最小修复并闭合重跑 | `ccf-mes-validation` |
| Phase-B 环境通过但算法失败后修改/修复算法并保留 anchor 回归 | `ccf-complexity-upgrade` |
