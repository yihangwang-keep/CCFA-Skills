...
For scenario-driven algorithm projects, register versioned artifact roots under `artifacts.phase_a`, `artifacts.phase_b`, `artifacts.environment_audit`, and `artifacts.algorithm_audit`. Phase A owns the problem document, candidate MES/environment, initial algorithm, repair rounds, and terminal anchor. Phase B owns each upgrade document, `stage_case`, environment/algorithm revisions, regressions, and repair rounds. Store only current pointers in project state; detailed contracts, digests, failures, gate evidence, and native reviews stay in their owning artifacts rather than `ccfa.yaml`.

Phase-A artifact fields use `minimum_executable_scenario`, `minimum_executable_scenario_version`, `mes_role`, and `mes_freeze_epoch`. Readers may interpret `scenario_mvp` and `scenario_mvp_version` as read-only aliases, but writers do not emit them. Phase-B artifacts use `stage_request_id`, `complexity_stage_id`, `parent_complexity_stage_id`, `stage_problem_version`, and `stage_case_id`; they keep only `anchor_mes_version` as an immutable Phase-A pointer and set `l2_scope: inherited_for_complexity_stage`. A Phase-B writer must not emit any candidate/successor/replacement MES field.
