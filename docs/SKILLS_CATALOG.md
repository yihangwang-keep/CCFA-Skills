...
| `ccf-idea-optimizer` | Idea | Turn rough directions into problem-gap-insight-method-evidence plans. | 优化粗 idea、具象化研究思路、形成 problem-gap-insight。 | idea shaping | Ranking multiple ideas as the main task. |
| `ccf-idea-reviewer` | Idea gate | Score, compare, rank, and triage early ideas. | idea 评分、选题排名、严格锐评、判断创新性和风险。 | idea scoring | Manuscript polishing or single-idea development. |
| `ccf-literature-searcher` | Evidence | Search and screen literature, prior art, datasets, benchmarks. | 检索相关工作、prior art、数据集、benchmark。 | search, screening | Auditing only already cited papers. |
...
| `ccf-paper-writer` | Manuscript | Draft, revise, polish, compress, preserve source format during edits, create venue-aware LaTeX manuscripts from ideas, and presentation-adapt paper text. | 写作、润色、压缩；保留原格式；只有 idea 时按目标会议 LaTeX 起草；缺省回退 NeurIPS。 | draft, polish, compression, venue-aware LaTeX drafting, presentation | Full review, evidence audit, package check, rebuttal. |
