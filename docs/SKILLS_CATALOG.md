...
| `ccf-experiment-env-design` | Environment formulation | Diagnose the scenario problem and causes; audit plan reasonableness, complexity balance, and whether tuned rules must trade one objective side against the other. | 审查场景问题和原因、计划合理性、复杂度平衡与可调规则是否被迫做 tradeoff。 | audit, design, repair, handoff-spec | Checking whether existing code faithfully implements the plan. |
| `ccf-experiment-env-code-auditor` | Environment implementation gate | Gate the plan, trace it into complete scenario code, and check whether tuned rules are forced to expose the intended tradeoff. | 先审查计划，再核验完整场景代码是否迫使启发式在 tradeoff 两侧之间取舍。 | static-trace, executable-audit, repair, acceptance-gate | Anything outside the current scenario plan, implementation, and execution evidence. |
...
| `ccf-experiment-debugger` | Experiment diagnosis | Diagnose failed, unstable, unexpectedly weak, or irreproducible solver/training experiments. | 实验失败、结果异常、训练发散、复现失败的代码优先诊断。 | code diagnosis, algorithm diagnosis, scenario diagnosis, controlled rerun | Initial environment implementation acceptance. |
...
- Communication formulation design goes to `ccf-experiment-env-design`; verifying that an existing environment implementation matches that formulation and resists simple shortcuts goes to `ccf-experiment-env-code-auditor`; failed solver or training runs go to `ccf-experiment-debugger`.
