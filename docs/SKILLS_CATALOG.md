...
| `ccf-env-design` | Environment design | Own the paper scenario, formal optimization problem, parameter applicability range, scenario MVP, information pattern, and feasibility meaning. | 设计并维护论文场景、正式优化问题、参数适用范围、场景 MVP、信息模式和可行性含义。 | design, audit, repair, handoff-spec | Environment-code validation or algorithm design. |
| `ccf-env-code-auditor` | Environment implementation gate | Audit authority, contract traceability, semantics, independent MVP execution, optimization fidelity, and central-tradeoff evidence. | 核验权威版本、契约追踪、语义、独立 MVP 执行、优化保真和中心权衡证据。 | static-trace, executable-audit, repair, acceptance-gate | Scenario design or algorithm performance. |
...
| `ccf-experiment-debugger` | Design validation / failure coordination | Repeat the versioned environment-algorithm validation loop, isolate one owner, route one minimal change, and close every affected rerun. | 运行版本化环境-算法验证闭环，定位单一 owner，执行一次最小修改并闭合全部受影响复审。 | diagnose, design-validation-loop, owner isolation, minimal repair, rerun closure | Replacing auditors or initial design. |
...
- The ordered design-validation loop is `ccf-env-design` -> `ccf-env-code-auditor` -> `ccf-algorithm-designer` -> `ccf-algorithm-code-auditor` -> `ccf-experiment-debugger` on failure. An environment semantic change creates a new problem version and invalidates every dependent algorithm, baseline, and result artifact.
