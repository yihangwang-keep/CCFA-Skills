...
推荐的使用方式不是一条机械直线，而是带有两个门控点的研究闭环：

```text
raw idea
  -> ccf-idea-optimizer             ：首轮问题 / 方法 / 证据成形
  -> ccf-idea-reviewer              ：问题-方法门控
       如果薄弱但可修复          ：回到 ccf-idea-optimizer 做定向修复
       如果根本不匹配            ：pivot 或停止投入
       如果已经可发展            ：交给 ccf-writing-skills
  -> ccf-writing-skills             ：建构论文论证和章节
  -> ccf-conference-paper-reviewer  ：投稿前审稿压力测试
       如果扣分点可修复          ：回到 ccf-writing-skills 修改论文
       如果真实审稿意见到来      ：使用 ccf-conference-paper-rebuttal
```

因此，第二次出现 `ccf-idea-optimizer` 不是重复。第一次 optimizer 负责把粗糙方向整理到“可以被判断”的状态；第二次只在 reviewer 发现可修复弱点时触发，用来定向修补问题、收窄主张或重新选择方法路线。`ccf-conference-paper-rebuttal` 也不是投稿前 reviewer 的替代品，它属于真实审稿意见到来之后的回应阶段。
