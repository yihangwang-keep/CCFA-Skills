...
这个项目的核心 insight 是：论文质量来自连续决策的质量。写作、评审、结论/证据审计、投稿检查和 rebuttal 不应该互相替代，而应该各自保持边界，并在同一个项目状态中交接。当前 v0.8 家族共有 21 个阶段角色，其中包含版本化的环境-算法设计验证闭环。每个阶段都有清楚的责任，每个 artifact 都能找到归属，整套系统更像一个围绕研究故事线展开的协作框架，而不是松散的 prompt 集合。v0.8 还为 `ccf-visual-composer` 增加了内置 Python SVG 绘图配方库，用于生成论文级图表示例。
...
项目搭建
  ...
  -> 文献监控 / 竞品跟踪
  -> 文献检索
  -> 论文场景、正式优化问题与核心 Tradeoff MES 设计
  -> 环境实现审计
  -> 算法设计
  -> 算法实现审计
  -> 冻结 MES 锚点后逐级拓展场景复杂度
...
通信场景和算法在论文范围实验设计前分为两个阶段，且两个阶段都有循环修复：第一阶段接受包含核心 Tradeoff 的 anchor MES，执行一次启发式 L2/algorithmic-need 检验并通过初始算法；初始算法失败时针对 anchor 修复算法。第二阶段由用户提出一次复杂度升级，检查阶段实现一致性和 anchor 回归，运行/升级算法并在失败时修复。第二阶段继承 anchor 的 L2 结果，不重新运行启发式检验，也不重设计 MES。

```text
ccf-env-design
  -> ccf-env-code-auditor                 [environment-valid + anchor MES/L2]
  -> ccf-algorithm-designer
  -> ccf-algorithm-code-auditor           [joint-ready]
  -> 用户复杂度升级：阶段实现审计 + anchor 回归
  -> ccf-experiment-debugger（初始算法/阶段失败时，条件性修复）
       -> 算法 owner、一次最小修改、重跑 anchor 回归与当前阶段
  -> ccf-experiment-designer（阶段通过后的论文范围使用证据）
```
...
| 环境设计 | `ccf-env-design` | 用户要定义论文场景、正式优化问题、参数适用范围和包含核心 Tradeoff 的最小但完整 MES，并在接受后冻结锚点、管理复杂度阶梯。 | 版本化环境规格、锚点 MES、复杂度阶段契约与算法可见契约。 | 环境代码验证或算法设计。 |
| 环境实现 gate | `ccf-env-code-auditor` | 用户要核验环境代码是否实现已接受问题并可独立运行。 | 设计到代码追踪、执行证据、`environment-valid` 判定。 | 重设计场景或判断算法性能。 |
| 算法设计 | `ccf-algorithm-designer` | 用户要在已接受环境契约上推导算法机制与算法 MVP。 | 算法规格、验证目标、复杂度分析。 | 重设计场景、审计代码或设计论文实验。 |
| 算法实现 gate | `ccf-algorithm-code-auditor` | 用户要核验算法规格、代码实现与独立 MVP 行为。 | 设计到代码追踪、参照比较、`joint-ready` 判定。 | 初始算法选择或环境审计。 |
| 阶段修复闭环 | `ccf-experiment-debugger` | 用户要修复已接受 anchor/复杂度阶段的失败运行。 | 单一 owner 修复、失效清单、anchor/阶段重跑、终止状态。 | 设计初始场景/算法或规划论文范围实验。 |
| 论文范围使用证据 | `ccf-experiment-designer` | 用户要基于已接受 anchor/阶段算法设计 baseline、metric、消融、鲁棒性和结论证据。 | 实验协议、baseline 矩阵、结果表模板、evidence-bound 图表规格。 | 定义/修复环境或算法语义、审计代码或编造结果。 |
...
| 设计论文场景与正式优化问题 | `ccf-env-design` | `ccf-algorithm-designer` |
| 核验环境代码是否实现已接受问题 | `ccf-env-code-auditor` | `ccf-env-design` |
| 在已接受环境上设计算法 | `ccf-algorithm-designer` | `ccf-env-design` |
| 核验算法代码与 MVP 行为 | `ccf-algorithm-code-auditor` | `ccf-algorithm-designer` |
| 协调失败 MVP 修复并闭合重跑 | `ccf-experiment-debugger` | `ccf-experiment-designer` |
...
| 优化图表排版、Python 绘图代码、创意数据分析图、配色、caption、多面板布局、正文嵌入 | `ccf-visual-composer` | `ccf-experiment-designer` |
...
| 组合 | 包含 | 适合 |
| --- | --- | --- |
| 全流程 | 21 个 runtime skills | 从 idea、版本化设计验证到 rebuttal 的完整论文项目。 |
| 通信设计验证子集 | `ccf-common`, `ccf-pipeline-orchestrator`, `ccf-env-design`, `ccf-env-code-auditor`, `ccf-algorithm-designer`, `ccf-algorithm-code-auditor`, `ccf-experiment-debugger` | 达到 `environment-valid` 和 `joint-ready`，并闭合失败修复。 |
...
| 早期研究子集 | `ccf-common`, `ccf-idea-optimizer`, `ccf-idea-reviewer`, `ccf-literature-monitor`, `ccf-literature-searcher`, `ccf-env-design`, `ccf-env-code-auditor`, `ccf-algorithm-designer`, `ccf-algorithm-code-auditor`, `ccf-experiment-debugger`, `ccf-experiment-designer` | 写正文前的 idea、文献、设计验证和实验设计。 |
| 图表/正文呈现子集 | `ccf-common`, `ccf-experiment-designer`, `ccf-visual-composer`, `ccf-paper-writer`, `ccf-integrity-auditor`, `ccf-submission-checker` | 基于真实结果制作论文图表、配色、caption、正文嵌入和一致性检查。 |
