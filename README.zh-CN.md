...
项目搭建
  ...
  -> ccf-mes-validation：最小但完整的 MES、场景审计、初始算法与修复
...
通信场景和算法分为两个独立阶段。Phase A 接受完整科学问题文档，依次实现最小但完整的 MES/环境、场景审计、初始算法和算法审计/修复，只有全部通过后才冻结 anchor。Phase B 读取已接受的 MES、现有代码和实验结果，先写一份升级场景文档，再直接按文档修改现有环境并审计，随后修改和修复算法；它不再设计新的 MES，也不要求先运行未经修改的原算法。
...
ccf-complexity-upgrade                    [Phase B：现有 MES/代码/结果 -> 升级场景文档 -> 修改现有环境 -> 算法修改/修复]
       -> 环境审计 + 算法审计；无新 MES、无需运行未经修改的原算法基线
```
