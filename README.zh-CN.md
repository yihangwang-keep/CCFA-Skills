...
### 一个面向 CCF-A 头脑风暴、研究选题、文献检索、实验设计、论文建构、严格写作评审与作者回应的研究辅助 skill 家族。
...
CCFA Skills 是一组面向 CCF-A 学术研究流程的 agent-readable research skills。它关注的是从非正式 idea 到可辩护投稿之间的关键阶段：澄清复杂需求、形成方法、联网检索并评分相关文献、校准新颖性、设计证据、组织和压缩论文、进行严格写作/格式评审，以及在审稿后进行回应。
...
| **Review Layer** | 在投稿前做逐段写作、LaTeX/格式、全文一致性和 reviewer-readable 评审。 | `ccf-conference-paper-reviewer` |
...
推荐的使用方式不是自由跳转，而是明确路由。`ccf-common/references/routing.md` 定义每类任务的归属，避免 brainstorming、idea 优化、idea 评分、文献检索、实验设计、论文写作、篇幅压缩、论文写作评审、rebuttal 和 skill 维护互相抢触发。
...
writing request
  -> ccf-writing-skills                           ：默认 writing-only
       ...
       论文写作评审按 handoff mode 执行           ：ccf-conference-paper-reviewer
...
这是一个保留旧目录名的 paper writing reviewer。它逐段阅读论文，检查故事线、LaTeX/格式、claim-evidence 对齐、全文一致性、图表叙事和贡献展示，并把每个问题转化为带位置的修改行动。
