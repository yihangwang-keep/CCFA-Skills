...
### 一个面向 CCF-A 研究选题、文献检索、实验设计、论文建构、审稿模拟与作者回应的研究辅助 skill 家族。
...
CCFA Skills 是一组面向 CCF-A 学术研究流程的 agent-readable research skills。它关注的是从非正式 idea 到可辩护投稿之间的关键阶段：澄清问题、形成方法、联网检索并评分相关文献、校准新颖性、设计证据、组织和压缩论文、预判审稿，以及在审稿后进行回应。
...
推荐的使用方式不是自由跳转，而是明确路由。`ccf-common/references/routing.md` 定义每类任务的归属，避免 idea 优化、idea 评分、文献检索、实验设计、论文写作、篇幅压缩、完整审稿、rebuttal 和 skill 维护互相抢触发。
...
raw idea
  ...
  -> ccf-idea-reviewer                            ：当用户要求评分/排序时做问题-方法门控
...
writing request
  -> ccf-writing-skills                           ：默认 writing-only
       ...
       完整论文 review 按 handoff mode 执行       ：ccf-conference-paper-reviewer
