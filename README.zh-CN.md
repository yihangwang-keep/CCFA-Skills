...
项目搭建
  ...
  -> ccf-mes-validation：候选 MES、一次 L2、初始算法与修复
  -> ccf-complexity-upgrade：冻结锚点后逐次拓展场景复杂度
  -> 失败 MVP 诊断与算法最小修复（按需）
  -> 实验设计
  -> 图表视觉整合
  -> 写作范例抽取（可选）
...
ccf-complexity-upgrade                    [Phase B：升级文档 -> stage_case 环境 -> 算法修改/修复；无新 MES]
       -> 阶段一致性 + anchor 回归 + 算法升级；失败时调用 debugger
```
