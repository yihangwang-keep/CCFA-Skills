...
### 一个面向 CCF-A 头脑风暴、研究选题、文献检索、实验设计、论文建构、完整科学审稿、严格写作评审与作者回应的研究辅助 skill 家族。
...
CCFA Skills 是一组面向 CCF-A 学术研究流程的 agent-readable research skills。它关注的是从非正式 idea 到可辩护投稿之间的关键阶段：澄清复杂需求、形成方法、联网检索并评分相关文献、校准新颖性、设计证据、组织和压缩论文、进行完整科学审稿、进行严格写作/格式评审，以及在审稿后进行回应。
...
| **Evidence Layer** | 检索高质量文献并设计不伪造结果的实验方案。 | `ccf-literature-search`, `ccf-experiment-designer` |
| **Manuscript Layer** | 将可行方向发展为连贯的 CCF-A 论文，并按篇幅限制压缩。 | `ccf-writing-skills`, `ccf-paper-compressor` |
| **Review Layer** | 在投稿前做完整科学审稿、模拟 reviewer、AC/meta-review，以及写作/LaTeX 评审。 | `ccf-conference-reviewer`, `ccf-conference-writing-reviewer` |
...
推荐的使用方式不是自由跳转，而是明确路由。`ccf-common/references/routing.md` 定义每类任务的归属，避免 brainstorming、idea 优化、idea 评分、文献检索、实验设计、论文写作、篇幅压缩、完整科学审稿、论文写作评审、rebuttal 和 skill 维护互相抢触发。
...
writing request
  -> ccf-writing-skills                           ：默认 writing-only
       ...
       完整科学审稿按 handoff mode 执行           ：ccf-conference-reviewer
...
在进入下游研究工作前澄清复杂请求。它把模糊目标整理成 bounded research-task brief：需要做的决定、目标受众、已有输入、约束、成功标准、工作流选项和推荐的下一步 CCFA skill。

它是按需使用的上游模块，不是全局硬门。适合用户要求头脑风暴、需求澄清、任务拆解、研究路线讨论或 design brief，然后再选择下游 skill。
...
在论文尚未形成之前，只评价问题和方法。标准模式会先用 public-safe 查询检索 closest related work，再对照已有工作判断 novelty delta，并使用领域、方法、实验、venue 和 skeptical prior-art 等多个专家视角。

它的目标不是奖励自信表达，也不是输出泛泛鼓励。它区分低新颖性与未知新颖性，区分可行性风险与 framing 风险，也区分可修复设计问题与需要 pivot 的根本原因。每个主要批评都必须绑定 idea claim、closest-work 或证据锚点、reviewer 扣分理由和具体修复条件。

### `ccf-literature-search`

联网检索高质量文献，按策略排除 MDPI，分类 paper type，评分文章质量，并写入包含标题、链接、评分、文章类型和备注的 literature-search 文件夹。

它主要联动 Related Work、Introduction、idea 优化、idea 评审、实验设计和 reviewer-risk 诊断。纯 benchmark 论文会单独标记，而不是按方法论文标准扣分。

### `ccf-experiment-designer`

设计 CCF-A 实验证据包：数据集、benchmark、baseline、ablation、metric、robustness test、failure analysis 和结果填写表。

它禁止伪造实验结果。缺少数值时，只提供给用户填写的表格模板，并标注每个实验回答哪个 reviewer 问题。

### `ccf-writing-skills`

将成熟 idea 发展为论文级论证。它处理故事线、章节规划、段落角色、claim-evidence map、venue adaptation、样例启发式写作策略和 score-risk 修改。
...
负责完整科学会议审稿。它执行 desk check、public-safe 相关工作搜索、新颖性/可靠性/证据评审、多 reviewer 模拟、AC/meta-review、校准分数、concerns table，并在 standard 模式生成固定格式 Markdown 审稿报告。

报告格式参考本地 CSPaper 风格，并扩展 claim-evidence audit、实验/可复现性审计、reviewer panel、AC 综合、score revision criteria 和 CCFA handoff actions。

### `ccf-conference-writing-reviewer`

这是 writing-only reviewer。它逐段阅读论文，检查故事线、LaTeX/格式、claim-evidence 展示、全文一致性、图表叙事和贡献展示，并把每个问题转化为带位置的写作修改行动。

它不负责完整科学审稿、AC/meta-review 或论文评分；这些请求路由到 `ccf-conference-reviewer`。
