...
這個專案的核心 insight 是：論文品質來自連續決策的品質。寫作、審稿、結論/證據稽核、投稿檢查和 rebuttal 不應該互相替代，而應該各自保持邊界，並在同一個專案狀態中交接。目前 v0.8 家族共有 19 個階段角色，其中包含分離的 MES 驗證與複雜度升級閉環。每個階段都有清楚的責任，每個 artifact 都能找到歸屬，整套系統更像一個圍繞研究故事線展開的協作框架，而不是鬆散的 prompt 集合。v0.8 也為 `ccf-visual-composer` 增加內建 Python SVG 繪圖配方庫，用於生成論文級圖表示例。
...
專案搭建
  ...
  -> 文獻監控 / 競品追蹤
  -> 文獻檢索
  -> 論文場景、正式最佳化問題與核心 Tradeoff MES 設計
  -> ccf-mes-validation：最小但完整的 MES、場景稽核、初始演算法與修復
  -> ccf-complexity-upgrade：凍結錨點後逐次拓展場景複雜度
  -> 實驗設計
  -> 圖表視覺整合
  -> 寫作範例抽取（可選）
  -> 會議感知寫作
...
通訊場景和演算法分成兩個獨立階段。Phase A 接受完整科學問題文件，依序實作最小但完整的 MES/環境、場景稽核、初始演算法和演算法稽核/修復，全部通過後才凍結 anchor。Phase B 讀取已接受的 MES、現有程式碼和實驗結果，先寫一份升級場景文件，再直接依文件修改現有環境並稽核，隨後修改和修復演算法；它不再設計新的 MES，也不要求先執行未修改的原演算法。

```text
ccf-mes-validation                        [Phase A：問題文件 -> candidate MES/環境 -> 初始演算法 -> anchor]
ccf-complexity-upgrade                    [Phase B：現有 MES/程式碼/結果 -> 升級場景文件 -> 修改現有環境 -> 演算法修改/修復]
       -> 環境稽核 + 演算法稽核；無新 MES、無需執行未修改的原演算法基線
  -> 只有獨立確認問題不可行/病態時才進入 formal-model review
```
...
| 流程/證據計畫 | `ccf-pipeline-orchestrator` | 使用者要拆任務、排階段、設 gate，或在階段通過後設計 baseline、metric、消融和穩健性證據。 | 階段計畫、handoff、結論-證據 ledger、TBD 結果計畫。 | 實作/稽核階段程式碼、編造結果、繪圖或寫正文。 |
...
| Phase A MES 驗證 | `ccf-mes-validation` | 使用者要從完整問題文件完成最小但完整的 MES/環境、初始演算法、稽核和修復閉環。 | 問題合同、環境/演算法實作、Phase-A 記錄和凍結 anchor。 | 後續場景升級或論文範圍證據計畫。 |
| 環境實作 gate | `ccf-env-code-auditor` | 使用者要核驗環境程式碼是否實作已接受問題並可獨立執行。 | 設計到程式碼追蹤、執行證據和環境稽核結果。 | 重設計場景或判斷演算法效能。 |
| 演算法實作 gate | `ccf-algorithm-code-auditor` | 使用者要核驗初始或升級演算法的規格、程式碼實作與獨立參照行為。 | 設計到程式碼追蹤、參照比較和演算法稽核結果。 | 演算法選擇、實作或環境稽核。 |
| Phase B 複雜度升級 | `ccf-complexity-upgrade` | 使用者要根據現有程式碼和結果寫升級場景文件，直接修改/稽核環境，再修改和修復演算法。 | 升級文件、環境/演算法增量、稽核和 Phase-B 記錄。 | 建立新 MES、先跑未修改的原演算法基線、修改 anchor 或論文範圍證據計畫。 |
...
| 組合 | 包含 | 適合 |
| --- | --- | --- |
| 全流程 | 19 個 runtime skills | 從 idea、分離的 MES/複雜度設計驗證到 rebuttal 的完整論文專案。 |
