...
通訊場景和演算法在論文範圍實驗設計前分為兩個階段，且兩個階段都有循環修復：第一階段接受包含核心 Tradeoff 的 anchor MES，執行一次啟發式 L2/algorithmic-need 檢驗並通過初始演算法；初始演算法失敗時針對 anchor 修復演算法。第二階段由使用者提出一次複雜度升級，檢查階段實作一致性和 anchor 回歸，執行/升級演算法並在失敗時修復。第二階段繼承 anchor 的 L2 結果，不重新執行啟發式檢驗，也不重設計 MES。
...
ccf-env-design
  ...
  -> 使用者複雜度升級：階段實作稽核 + anchor 回歸
  -> ccf-experiment-debugger（初始演算法/階段失敗時，條件性修復）
       -> 演算法 owner、一次最小修改、重跑錨點回歸與當前階段
  -> ccf-experiment-designer（階段通過後的論文範圍使用證據）
...
| 環境設計 | `ccf-env-design` | 使用者要定義論文場景、正式最佳化問題、參數適用範圍和包含核心 Tradeoff 的最小但完整 MES，接受後凍結錨點並管理複雜度階梯。 | 版本化環境規格、錨點 MES、複雜度階段契約與演算法可見契約。 | 環境程式碼驗證或演算法設計。 |
...
| 階段修復閉環 | `ccf-experiment-debugger` | 使用者要修復已接受 anchor/複雜度階段的失敗執行。 | 單一 owner 修復、失效清單、anchor/階段重跑、終止狀態。 | 設計初始場景/演算法或規劃論文範圍實驗。 |
| 論文範圍使用證據 | `ccf-experiment-designer` | 使用者要基於已接受 anchor/階段演算法設計 baseline、metric、消融、魯棒性和結論證據。 | 實驗協議、baseline 矩陣、結果表模板、evidence-bound 圖表規格。 | 定義/修復環境或演算法語義、稽核程式碼或編造結果。 |
