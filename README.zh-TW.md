...
專案搭建
  ...
  -> ccf-mes-validation：候選 MES、一次 L2、初始演算法與修復
...
通訊場景和演算法分成兩個獨立階段。Phase A 接受完整科學問題文件，依序實作 candidate MES/環境、執行一次 L2、實作並稽核初始演算法，全部通過後才凍結 anchor。Phase B 接受版本化複雜度升級文件，先實作並稽核 `stage_case` 環境，再執行、修改和修復演算法；它繼承 L2、保留 anchor 回歸，而且不再建立 MES。

```text
ccf-mes-validation                        [Phase A：問題文件 -> candidate MES/環境 -> 初始演算法 -> anchor]
ccf-complexity-upgrade                    [Phase B：升級文件 -> stage_case 環境 -> 演算法修改/修復；無新 MES]
       -> 階段一致性 + anchor 回歸 + 演算法升級；失敗時呼叫 debugger
...
| Phase A MES 驗證 | `ccf-mes-validation` | 使用者要從完整問題文件完成候選 MES/環境、一次 L2、初始演算法和修復閉環。 | 問題合同、環境/演算法實作、Phase-A 記錄和凍結 anchor。 | 後續複雜度升級或論文範圍證據計畫。 |
...
| Phase B 複雜度升級 | `ccf-complexity-upgrade` | 使用者提供升級文件，要實作/稽核複雜場景，再修改和修復演算法。 | 升級合同、`stage_case`、環境/演算法增量、anchor 回歸和 Phase-B 記錄。 | 建立新 MES、重跑 L2、修改 anchor 或論文範圍證據計畫。 |
