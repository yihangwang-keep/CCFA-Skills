...
### 一個面向 CCF-A 頭腦風暴、研究選題、文獻檢索、實驗設計、論文建構、完整科學審稿、嚴格寫作評審與作者回應的研究輔助 skill 家族。
...
CCFA Skills 是一組面向 CCF-A 學術研究流程的 agent-readable research skills。它關注的是從非正式 idea 到可辯護投稿之間的關鍵階段：澄清複雜需求、形成方法、聯網檢索並評分相關文獻、校準新穎性、設計證據、組織和壓縮論文、進行完整科學審稿、進行嚴格寫作/格式評審，以及在審稿後進行回應。
...
| **Review Layer** | 在投稿前做完整科學審稿、模擬 reviewer、AC/meta-review，以及寫作/LaTeX 評審。 | `ccf-conference-reviewer`, `ccf-conference-writing-reviewer` |
...
推薦的使用方式不是自由跳轉，而是明確路由。`ccf-common/references/routing.md` 定義每類任務的歸屬，避免 brainstorming、idea 優化、idea 評分、文獻檢索、實驗設計、論文寫作、篇幅壓縮、完整科學審稿、論文寫作評審、rebuttal 和 skill 維護互相搶觸發。
...
writing request
  -> ccf-writing-skills                           ：預設 writing-only
       ...
       完整科學審稿按 handoff mode 執行           ：ccf-conference-reviewer
...
在進入下游研究工作前澄清複雜請求。它把模糊目標整理成 bounded research-task brief：需要做的決定、目標受眾、既有輸入、約束、成功標準、工作流選項和推薦的下一步 CCFA skill。

它是按需使用的上游模組，不是全局硬門。適合使用者要求頭腦風暴、需求澄清、任務拆解、研究路線討論或 design brief，然後再選擇下游 skill。
...
在論文尚未形成之前，只評價問題和方法。標準模式會先用 public-safe 查詢檢索 closest related work，再對照既有工作判斷 novelty delta，並使用領域、方法、實驗、venue 和 skeptical prior-art 等多個專家視角。

它的目標不是獎勵自信表達，也不是輸出泛泛鼓勵。它區分低新穎性與未知新穎性，區分可行性風險與 framing 風險，也區分可修復設計問題與需要 pivot 的根本原因。每個主要批評都必須綁定 idea claim、closest-work 或證據錨點、reviewer 扣分理由和具體修復條件。
...
負責完整科學會議審稿。它執行 desk check、public-safe 相關工作搜尋、新穎性/可靠性/證據評審、多 reviewer 模擬、AC/meta-review、校準分數、concerns table，並在 standard 模式生成固定格式 Markdown 審稿報告。

報告格式參考本地 CSPaper 風格，並擴展 claim-evidence audit、實驗/可復現性審計、reviewer panel、AC 綜合、score revision criteria 和 CCFA handoff actions。

### `ccf-conference-writing-reviewer`

這是 writing-only reviewer。它逐段閱讀論文，檢查故事線、LaTeX/格式、claim-evidence 展示、全文一致性、圖表敘事和貢獻展示，並把每個問題轉化為帶位置的寫作修改行動。

它不負責完整科學審稿、AC/meta-review 或論文評分；這些請求路由到 `ccf-conference-reviewer`。
