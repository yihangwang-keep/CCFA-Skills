...
推薦的使用方式不是自動流水線，而是 gated workflow。每一條進入其他 skill 的箭頭都是一個可選模組門：如果使用者沒有明確要求使用該 skill，就應先詢問；如果使用者明確禁用，就必須切換到本模組內部的 fallback。

```text
raw idea
  -> [詢問/啟用後] ccf-idea-optimizer             ：問題 / 方法 / 證據成形
  -> [詢問/啟用後] ccf-idea-reviewer              ：問題-方法門控
       如果薄弱但可修復且 optimizer 啟用         ：回到 optimizer 做定向修復
       如果根本不匹配                            ：pivot 或停止投入
       如果已經可發展且 writing 啟用             ：進入寫作模組

writing request
  -> ccf-writing-skills                           ：預設 writing-only
       修改 idea scope 需要明確確認              ：否則只標註 Idea-level risk
       投稿前 review 是可選模組                  ：呼叫 ccf-conference-paper-reviewer 前先詢問

真實審稿意見到來
  -> ccf-conference-paper-rebuttal                ：作者回應和修改承諾
       改論文或評估 rebuttal 影響                ：呼叫可選模組前先詢問
```
...
因此，第二次出現 `ccf-idea-optimizer` 不是重複，也不是自動執行。第一次 optimizer 負責把粗糙方向整理到「可以被判斷」的狀態；後續只有在 reviewer 診斷後且使用者允許再次啟用 optimizer 時，才會進入定向修復。`ccf-conference-paper-rebuttal` 也不是投稿前 reviewer 的替代品，它屬於真實審稿意見到來之後的回應階段。
