...
### 一個面向 CCF-A 頭腦風暴、研究選題、文獻檢索、實驗設計、論文建構、嚴格寫作評審與作者回應的研究輔助 skill 家族。
...
CCFA Skills 是一組面向 CCF-A 學術研究流程的 agent-readable research skills。它關注的是從非正式 idea 到可辯護投稿之間的關鍵階段：澄清複雜需求、形成方法、聯網檢索並評分相關文獻、校準新穎性、設計證據、組織和壓縮論文、進行嚴格寫作/格式評審，以及在審稿後進行回應。
...
| **Review Layer** | 在投稿前做逐段寫作、LaTeX/格式、全文一致性和 reviewer-readable 評審。 | `ccf-conference-paper-reviewer` |
...
推薦的使用方式不是自由跳轉，而是明確路由。`ccf-common/references/routing.md` 定義每類任務的歸屬，避免 brainstorming、idea 優化、idea 評分、文獻檢索、實驗設計、論文寫作、篇幅壓縮、論文寫作評審、rebuttal 和 skill 維護互相搶觸發。
...
writing request
  -> ccf-writing-skills                           ：預設 writing-only
       ...
       論文寫作評審按 handoff mode 執行           ：ccf-conference-paper-reviewer
...
這是一個保留舊目錄名的 paper writing reviewer。它逐段閱讀論文，檢查故事線、LaTeX/格式、claim-evidence 對齊、全文一致性、圖表敘事和貢獻展示，並把每個問題轉化為帶位置的修改行動。
