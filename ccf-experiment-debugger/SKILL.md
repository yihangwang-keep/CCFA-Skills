...
Coordinate existing owners and auditors; do not create a third domain audit. Freeze the failed evidence and current artifact set, find the earliest confirmed owner, apply one owned delta, and rerun the original failure plus every invalidated check. This skill is the operational repair loop for an accepted anchor or user-requested complexity stage; it does not design the initial environment, choose the initial algorithm, or design publication-range experiments.
...
algorithm route exhausted -> ccf-env-design environment/formal-model review
  ...
  -> model_defect: authorize a non-simplifying formal-problem amendment and new candidate MES
  -> unresolved: block without changing the environment
```

The order controls permission, not blame. One weak algorithm result at a complexity stage stays algorithm-side. The initial anchor L2 heuristic-probe decision is upstream evidence and is not rerun by this loop. When environment L1 and algorithm-code fidelity are current but documented algorithm mechanism/family repairs still cannot pass, the debugger must open an environment/formal-model review. Only a confirmed model defect may change the formal authority and start a new candidate-MES epoch; routine complexity failure may never replace the frozen anchor or simplify the research problem.
...
1. **Failure authority:** preserve the command, first divergence, authority versions, frozen anchor MES, complexity stage, implementation revisions, validation contract, configuration, seeds or traces, logs, and outputs in a digested artifact set.
...
3. **Environment L2:** for initial anchor acceptance, require a frozen, method-independent anchor MES and adequately tuned, budget-matched heuristic probes. If a probe reaches the predeclared joint target, keep any valid L1 verdict but set `algorithmic_need: not_demonstrated` and stop the algorithm-contribution route; never change the anchor MES to force probe failure. For a later complexity stage, inherit the anchor L2 result and use stage L1 consistency plus anchor regression; do not rerun heuristic probes.
4. **Algorithm implementation:** after initial anchor L1/L2 acceptance, or the inherited anchor result plus stage L1/anchor-regression acceptance for a complexity upgrade, require current algorithm traceability, semantics, reference evidence, no-heuristic verification for `method_role: proposed`, and complete stage execution.
...
6. **Environment and mathematical model:** an exhaustion record from a complexity stage triggers `ccf-env-design` review. Keep the anchor and failed stage while the review runs. Accept a change only when evidence classifies a model defect, then version the corrected formal problem and new candidate MES/evidence epoch. Reject weakening done for tractability or acceptance. A scenario-inconsistent item may be corrected only on independent evidence and must retain the causal difficulty and central tradeoff.
...
5. Do not remove a failed case, retune validation after seeing outcomes, expose audit-only information, or modify the environment to favor a method. A complexity stage must preserve the anchor cases and be expanded one dimension at a time. Environment/model revision after exhausted algorithm repair must preserve the paper scenario's causal question and all material difficulty. A material authority change creates a new candidate version and invalidates dependent evidence; it is an exception, not an algorithm upgrade.
