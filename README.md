...
项目搭建
  ...
  -> 文献监控 / 竞品跟踪
  -> 文献检索
  -> ccf-mes-validation：完整问题文档、候选 MES/环境、一次 L2、初始算法、修复与 anchor 冻结
  -> ccf-complexity-upgrade：升级文档、stage_case 环境审计、算法修改/修复与 anchor 回归；不再创建 MES
...
通信场景和算法分为两个独立阶段。Phase A 接受完整科学问题文档，依次实现 candidate MES/环境、执行一次 L2、实现并审计初始算法，只有全部通过后才冻结 anchor。Phase B 接受一个版本化复杂度升级文档，先实现并审查 `stage_case` 环境，再运行、修改和修复算法；它继承 L2、保留 anchor 回归，并且不再创建 MES。

```text
ccf-mes-validation                        [Phase A：问题文档 -> candidate MES/环境 -> 初始算法 -> anchor]
  -> ccf-env-code-auditor / ccf-algorithm-code-auditor
ccf-complexity-upgrade                    [Phase B：升级文档 -> stage_case 环境 -> 算法修改/修复]
  -> 两个独立 auditor + anchor 回归；无新 MES、无 L2 重跑
```
...
| Phase A MES 验证 | `ccf-mes-validation` | 用户要从完整问题文档完成候选 MES/环境、一次 L2、初始算法和修复闭环。 | 问题合同、环境/算法实现、Phase-A 记录和冻结 anchor。 | 后续复杂度升级或论文范围证据计划。 |
...
| Phase B 复杂度升级 | `ccf-complexity-upgrade` | 用户提供升级文档，要实现/审查复杂场景，再修改和修复算法。 | 升级合同、`stage_case`、环境/算法增量、anchor 回归和 Phase-B 记录。 | 创建新 MES、重跑 L2、修改 anchor 或论文范围证据计划。 |
...
| 在冻结 anchor 上按升级文档实现复杂场景并修改/修复算法 | `ccf-complexity-upgrade` | `ccf-mes-validation` |
