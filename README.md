...
ccf-env-design
  ...
  -> ccf-experiment-debugger（失败时）
       -> 单一 owner、一次最小修改、重跑全部失效 gate
```
...
| 环境设计 | `ccf-env-design` | 用户要定义或修订论文场景、正式优化问题、参数适用范围、场景 MVP、信息模式和可行性含义。 | 版本化环境规格与算法可见契约。 | 环境代码验证或算法设计。 |
...
| 设计验证 | `ccf-experiment-debugger` | 用户要诊断失败/偏弱 MVP，或持续运行版本化设计验证闭环。 | 单一 owner 修复、失效清单、闭环复审、终止状态。 | 替代两个 auditor 或设计初始论文实验。 |
