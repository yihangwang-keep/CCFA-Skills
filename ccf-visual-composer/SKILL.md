...
- `python-plotting`: write or adapt Python plotting code using bundled recipes, standard-library SVG output, or optional libraries available in the user's environment.
...
6. For plotting-code requests, load `references/python-plot-recipes.md` and use `resources/python/ccfa_plot_recipes.py` as a runnable starting point. If a better plot grammar is needed, load `references/plot-inspiration-map.md` and invent a new evidence-bound chart without copying external code.
