...
For scenario-driven algorithm papers, keep this stage order: define the paper scenario and formal problem; derive and audit its scenario MVP; design the algorithm MVP; add any algorithm-required environment information and apply the same environment audit; audit the algorithm MVP; diagnose and version any necessary algorithm or scenario changes; then hand validated MVP evidence to experiment design for paper-scope evaluation.
...
5. Define the gate: required input, output artifact, pass condition, blocker, and handoff. For scenario/algorithm gates, also state the paper-scenario and MVP versions and which checks must rerun after a change.
