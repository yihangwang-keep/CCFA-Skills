...
description: "Design communication-paper usage evidence after accepted paper-scenario, frozen anchor MES, complexity-stage, environment, and algorithm-MVP evidence: range coverage, settings, baselines, metrics, mechanism tests, robustness, and result templates. Use for experiment design, baseline selection, ablations, result tables, 设计实验, 对比实验, 消融, 结果图表. This is downstream evidence/use planning, not environment or algorithm design/audit, and it does not invent results or repair upstream contracts."
metadata:
  ccf_skill_controls:
...
Consume accepted upstream evidence; do not recreate environment or algorithm design inside experiment planning. Map each intended paper conclusion to the smallest discriminating evidence set. Keep the paper scenario, formal problem, parameter range, frozen anchor MES, complexity-stage lineage, objective, constraints, information pattern, and feasibility meaning version-consistent.

This skill owns how an accepted environment/algorithm is used to produce paper-range evidence: settings, baselines, metrics, ablations, robustness, failure coverage, result tables, and figures. It does not own the initial MES, user-requested stage semantics, algorithm mechanism, code correctness, or failure repair. Route those objects to `ccf-env-design`, `ccf-algorithm-designer`, their code auditors, or `ccf-experiment-debugger`.
...
- paper scenario, formal optimization problem, parameter applicability range, frozen anchor MES, and current complexity-stage lineage;
- environment contract-fidelity verdict, anchor-only heuristic tradeoff-resistance result, and current stage consistency/regression result from `ccf-env-code-auditor`;
...
For a later complexity stage, require the user/project-owner `stage_request_id`, current stage implementation-consistency evidence, anchor regression, and the inherited anchor-only `algorithmic_need` record. A fresh L2 heuristic-probe sweep is neither required nor permitted for stage acceptance.
...
4. Apply the Scenario Integrity Gate. Ordinary paper-range evidence may sample new settings only within the accepted formal problem, generator, frozen anchor MES contract, accepted complexity-stage contract, and parameter range. A user-requested core stage must already be specified by `ccf-env-design`; this skill executes/plans its evidence but does not define it. Each stage keeps anchor regression cases and does not create an MES successor. Route a proposed core scenario or semantic change to `ccf-env-design`.
...
10. Treat the anchor `algorithmic_need` result as fixed input. Do not rerun the environment L2 heuristic-probe need test for a later complexity stage; heuristic methods may still appear as labelled paper baselines under matched conditions.
11. Route publication-grade figure/table composition, captions, manuscript placement, and render QA to `ccf-visual-composer`.
